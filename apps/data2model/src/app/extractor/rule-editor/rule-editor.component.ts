...
export class RuleEditorComponent implements OnInit, AfterViewInit {
    ...
    readonly relationTypes = Object.entries(toHumanReadableNames(archimate3.relations));
    ...
    ngOnInit() {
        this.currentDefinitionType$ = this.extractorService.select('currentDefinitionType');
        this.headers$ = this.extractorService.select('currentHeaders').pipe(shareReplay());
        this.elementRules$ = this.extractorService.elementRules.pipe(shareReplay());
        this.relationRules$ = this.extractorService.relationRules.pipe(shareReplay());
        ...
        this.extractorService.relationRules.subscribe(() => this.resetRelationForm());
        this.extractorService.viewRules.subscribe(() => this.resetViewForm());

        // Whenever a form clear is triggered, reset that form
        this.clearRule$
            .pipe(withLatestFrom(this.extractorService.select('currentDefinitionType')))
                    ...
                    if (this.validateForm(form)) {
                        ...
                        switch (action) {
                            ...
                            case SaveAction.SAVE_AS:
                                return of(Object.assign(form.value, { id: null, type: definitionType }));
                        }
                    } else {
                        return EMPTY;
                    }
        ...
        this.deleteRule$
            ...
            .subscribe(([, definitionType]) => {
                const form = this.getFormForType(definitionType);
                this.extractorService.deleteRule(Object.assign(form.value, { type: definitionType }));
    ...
    private initForms() {
        this.elementForm = this.formBuilder.group({
            ...
            concept_type: this.formBuilder.control(null, [Validators.required, typeIsKnown]),
