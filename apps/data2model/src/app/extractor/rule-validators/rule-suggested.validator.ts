...
export class ElementNotSuggestedError implements ValidationError {
    public readonly severity: ValidationErrorSeverity = ValidationErrorSeverity.INFO;
    ...
    get description() {
        return `Column ${this.referencedColumn} used as this element's identifier does not match any suggestion.`;
    }
}
...
export function ruleSuggested(
    ...
    const rulesWithoutSuggestion = rules
        .filter((rule) => rule.include && [DefinitionType.ELEMENT, DefinitionType.RELATION].includes(rule.type))
        .reduce((acc, rule) => {
            switch (rule.type) {
                case DefinitionType.ELEMENT:
                    if (rule.id_type === 'dynamic' && !columns.includes(rule.id_key)) {
                        return [...acc, new ElementNotSuggestedError(rule.id, rule.id_key)];
                    }
                    return acc;
                case DefinitionType.RELATION:
                    if (!transitions.includes([rule.source, rule.target])) {
                        return [...acc, new RelationshipNotSuggestedError(rule.id, rule.source, rule.target)];
                    }
                    return acc;
                default:
                    return acc;
