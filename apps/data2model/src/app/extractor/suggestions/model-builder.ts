...
export class ModelBuilder extends BasicStore<ModelBuilderContext> {
    ...
    private dependentRules$: Subject<DependentRules> = new BehaviorSubject<DependentRules>({});
    private errors$: Subject<ValidationResult[]> = new BehaviorSubject<ValidationResult[]>([]);
    ...
    constructor(
        ...
        Promise.all([this.createModelForm(this.id), this.createViewForm(this.id)]).then(() => this.init());
    }
    ...
    private init() {
        ...
        combineLatest([this.dependentRules$, this.extractorService.select('validationResult')])
            .pipe(
                map(([dependentRules, validationResult]) =>
                    Object.values(dependentRules).reduce(
                        (errors, ruleId) => [...errors, ...(validationResult[ruleId] || [])],
                        [] as ValidationResult[],
                    ),
                ),
            )
        ...
        this.dependentRules$
            .pipe(
                switchMap(({ source }) => this.extractorService.select(['rulesContext', 'rules', source])),
                distinctUntilChanged(isEqual),
            )
        ...
        this.dependentRules$
            .pipe(
                switchMap(({ target }) => this.extractorService.select(['rulesContext', 'rules', target])),
                distinctUntilChanged(isEqual),
            )
        ...
        this.dependentRules$
            .pipe(
                switchMap(({ relation }) => this.extractorService.select(['rulesContext', 'rules', relation])),
                distinctUntilChanged(isEqual),
            )
    ...
    toggleReversed(relationshipId: string, currentState: boolean) {
        this.update({
            description: `Set reversed status for relationship ${relationshipId} to ${Boolean(!currentState)
                .toString()
                .toUpperCase()}`,
    ...
    private async updateModelRules(relationshipId: string, context: ModelFormContext) {
        const [
        ...
        const updatedRelationshipRule = {
            ...
            id_prefix:
                relationRule && relationRule.id_type === 'dynamic' && relationRule.id_prefix
                    ? relationRule.id_prefix
                    : `relationship_${relationshipId}`,
            id_key:
                bijacentIdKey ||
                (context.relation.name
                    ? [sourceColumn as string, context.relation.name, targetColumn as string]
            ...
            source: reversed ? updatedTargetColumnRule.id : updatedSourceColumnRule.id,
    ...
    private async updateViewRules(relationshipId: string, context: ViewFormContext) {
        const [reversed, { source, target, view }, { source: sourceColumn, target: targetColumn }] = await Promise.all([
            this.get('reversed'),
            this.dependentRules$.pipe(first()).toPromise(),
            this.extractorService.get(['suggestions', 'relations', relationshipId]),
        ]);
    ...
    private async createModelForm(relationshipId: string) {
        ...
        const [{ source, target, relation }] = await Promise.all([this.dependentRules$.pipe(first()).toPromise()]);
        ...
        const modelForm = new UntypedFormGroup({
            relation: new UntypedFormGroup({
                ...
                type: new UntypedFormControl(
                    relationRule ? (relationRule as RelationshipDefinition).relationship_type : 'o',
                    [Validators.required, typeIsKnown],
                ),
            }),
            source: new UntypedFormGroup({
                type: new UntypedFormControl(
                    sourceRule ? (sourceRule as ElementDefinition).concept_type : 'ar3_Unknown',
                    [Validators.required, typeIsKnown],
                ),
            }),
            target: new UntypedFormGroup({
                type: new UntypedFormControl(
                    targetRule ? (targetRule as ElementDefinition).concept_type : 'ar3_Unknown',
                    [Validators.required, typeIsKnown],
                ),
            }),
        });
    ...
    private async createViewForm(relationshipId: string) {
        ...
        const existingViewRule = await this.extractorService.get(['rulesContext', 'rules', view], {
            includeFalsy: true,
        });
        ...
        const viewForm = new UntypedFormGroup(
            {
                ...
                layout: new UntypedFormControl(ViewLayout.ARCHIMATE_HIERARCHICAL, [Validators.required]),
            },
            [propertyKeyDefined],
        );
    ...
    private async handleGeneratePreviewModel(relationshipId: string) {
        const [data, rules, source, target] = await Promise.all([
            this.extractorService.get('currentDataset'),
            this.extractorService.get(['rulesContext', 'rules']),
            this.extractorService.get(['suggestions', 'relations', relationshipId, 'source']),
            this.extractorService.get(['suggestions', 'relations', relationshipId, 'target']),
        ]);
        ...
        const lookupColumnRule = async (columnName: string) => {
            const ruleId = await this.extractorService.get(['columnRuleMapping', columnName]);
            return rules[ruleId];
        };
        ...
        if (sourceColumnRule && targetColumnRule) {
            ...
            const viewRule = Object.values(rules).find(
                (rule) =>
                    ...
                    rule.view_nodes.map((node) => node.rule).includes(sourceColumnRule.id) &&
            ...
            if (relationshipRule && viewRule) {
                ...
                const { model, metadata } = await this.flaskService
                    .extract(data.slice(0, 5), {
                        elements: [sourceColumnRule, targetColumnRule] as ElementDefinition[],
