...
const suggestionsTableConfig: SortableTableShellConfig = {
    index: {
        displayName: '#',
        description: 'Suggestions are numbered by relevance, with #1 being the most relevant',
    },
    source: {
        displayName: 'Source',
        description: 'The name of the column which defines the origin of the relationship',
    },
    target: {
        displayName: 'Target',
        description: 'The name of the column which defines the destination of the relationship',
    },
    errors: {
        isNarrow: true,
        isStatic: true,
...
export class SuggestionsComponent implements OnInit, OnDestroy {
    readonly elementTypes = Object.entries(toHumanReadableNames(archimate3.elements));
    readonly relationshipTypes = Object.entries(toHumanReadableNames(archimate3.relations));
    ...
    ngOnInit() {
        const relationships$ = this.extractorService.select(['suggestions', 'relations']).pipe(
            map((relationships) => Object.values(relationships)),
            shareReplay(1),
        );

        this.dynamicRelations$ = relationships$.pipe(
            map((relations) => relations.filter((relation) => relation.type === 'dynamic'), shareReplay()),
        );
        ...
        this.structuralRelations$ = relationships$.pipe(
            map((relations) => relations.filter((relation) => relation.type === 'structural')),
            shareReplay(),
        );
        ...
        this.isCalculatingSuggestions$ = this.extractorService.select('isCalculatingSuggestions');
        ...
        this.isUpdatingModelRules$
            .pipe(untilDestroyed(this))
            .subscribe((state) => log.debug(`Is updating model rules: ${Boolean(state).toString().toUpperCase()}`));

        this.isUpdatingViewRules$ = this.suggestionsService
            .select('isUpdatingViewRules', { includeFalsy: true })
            .pipe(shareReplay());

        this.suggestedRelations$ = this.extractorService.select(['suggestions', 'relations']);
    ...
    async selectModelBuilderTab(relationshipId: string, tab: ModelBuilderTab) {
        const modelBuilder = await this.suggestionsService.get(['modelBuilder', relationshipId]);
