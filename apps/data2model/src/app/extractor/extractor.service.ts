...
export class ExtractorService extends BasicStore<ExtractorStoreContext> {
    private readonly commit$: Subject<ModelCommitContext> = new Subject<ModelCommitContext>();
    private readonly extract$: Subject<void> = new Subject<void>();
    private readonly loadRules$: Subject<File> = new Subject<File>();
    private readonly ruleSelected$: Subject<ExtractorRule> = new Subject<ExtractorRule>();
    ...
    private init() {
        ...
        this.select('currentDataset')
            .pipe(
                ...
                map((dataset) =>
                    Array.from(
                        dataset.reduce((headers: Set<string>, row: ExtractorDatasetEntry) => {
        ...
        combineLatest([this.select('currentDataset'), this.select('currentHeaders')]).subscribe(([dataset, headers]) =>
        ...
        this.commit$.pipe(exhaustMap((context) => this.handleCommit(context))).subscribe();

        // Whenever load rules is triggered, load the rules
        this.loadRules$.pipe(exhaustMap((rules) => this.handleLoadRules(rules))).subscribe();
        ...
        this.elementRules.pipe(switchMap((elements) => this.updateColumnRuleMapping(elements))).subscribe();
        ...
        this.select('rulesContext')
            ...
            .subscribe((rules) =>
                this.update({
                    ...
                    payload: {
                        currentRulesFileName: `d2m - ${rules.name} - ${new Date().toLocaleString()}.json`,
                        currentRulesObjectURL: this.sanitizer.bypassSecurityTrustUrl(
                            `data:text/json;charset=UTF-8,${encodeURIComponent(JSON.stringify(rules))}`,
                        ),
                    },
                }),
            );
    ...
    private async handleCommit(context: ModelCommitContext): Promise<any> {
        ...
        return this.modelCommitService.commitJsonModel(context.branchName, context.comment, model, {
    ...
    private async handleExtract(data: Dictionary<any>[], rules: ExtractorRule[]) {
        const { metadata, model } = await this.flaskService
            .extract(data, {
                elements: rules.filter((rule) => rule.type === DefinitionType.ELEMENT) as ElementDefinition[],
                relations: rules.filter((rule) => rule.type === DefinitionType.RELATION) as RelationshipDefinition[],
                views: rules.filter((rule) => rule.type === DefinitionType.VIEW) as ViewDefinition[],
            })
            .toPromise();
    ...
    private async validateRules(
        ...
        const [duplicateElements, duplicateRelations, duplicateViews] = await Promise.all([
            this.findDuplicates(elementRules),
            this.findDuplicates(relationRules),
            this.findDuplicates(viewRules),
        ]);

        const duplicates: ValidationResult[] = [...duplicateElements, ...duplicateRelations, ...duplicateViews].map(
            (duplicate) => ({
        ...
        const [intersectingElements, intersectingRelations, intersectingViews] = await Promise.all([
        ...
        const unmappedSources = relationRules
            .filter(
                (relationRule) =>
                    ...
                    undefined === elementRules.find((elementRule) => elementRule.id === relationRule.source),
            )
            .map((relation) => ({
                type: ValidationSeverityType.ERROR,
                rule: relation.id,
        ...
        const unmappedTargets = relationRules
            .filter(
                (relationRule) =>
                    elementRules.length === 0 ||
                    undefined === elementRules.find((elementRule) => elementRule.id === relationRule.target),
            )
            .map((relation) => ({
                type: ValidationSeverityType.ERROR,
                rule: relation.id,
            ...
            .filter(
                (viewRule) =>
                    ...
                    !!viewRule.view_nodes.find(
                        (node) => undefined === elementRules.find((elementRule) => elementRule.id === node.rule),
                    ),
            )
            .map((node) => ({
                type: ValidationSeverityType.ERROR,
            ...
            .filter(
                (viewRule) =>
                    ...
                    viewRule.view_edges.find(
                        (edge) => undefined === relationRules.find((relationRule) => relationRule.id === edge.rule),
                    ),
            )
            .map((edge) => ({
                type: ValidationSeverityType.ERROR,
            ...
            rules
                ...
                .find(
                    (other) =>
                        ...
                        ((!rule.id_prefix && !other.id_prefix) || rule.id_prefix === other.id_prefix),
                ),
        );
    }

    private async findIntersections(rules: ExtractorRule[], ...others: ExtractorRule[]) {
    ...
    private async updateColumnRuleMapping(elements: ElementDefinition[]) {
        const columnRuleMapping = elements
            .filter((element: any) => element.id_key && element.id_prefix && element.id_prefix.includes('element_'))
            .reduce(
                (mapping, element: any) => ({
                    ...mapping,
    ...
    private indexValuesPerColumn(dataset: ExtractorDatasetEntry[], headers: string[]) {
