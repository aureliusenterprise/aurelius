...
export async function bijacencyMap({ data, columns, source, target, name, valuesPerColumn }: BijacencyMapContext) {
    ...
    const identifiers = data.map((row) =>
        [row[source], row[target], row[name]].filter((value) => value !== undefined && value !== null),
    );
    ...
    const bijacencies = await Promise.all(
        columns
            ...
            .map(async (label) => ({
                label: label,
                value: await bijacency(valuesPerColumn[label.column], identifierSet.items, {
