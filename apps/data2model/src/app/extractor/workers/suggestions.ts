...
export async function suggestions<T extends Dictionary<any>>({
    ...
    await Promise.all(
        data.map(
            async (row) => await Promise.all(Object.keys(row).map(async (header) => keySet.add(header as keyof T))),
        ),
    );
    ...
    const pairs = await Promise.all(keys.map(async (key) => [key, await entropy(data, key)] as [string, number]));
    ...
    const filteredKeys = keys.filter((key) => maxEntropy * lowerEntropyBound <= entropyMap[key as string]);

    // Determine which columns contain strings
    const stringColumns = filteredKeys.filter(
        (key) => data.length === 0 || typeof data.find((row) => !!row[key])[key] === 'string',
    );
    ...
    const nameColumns = stringColumns.sort((a, b) => entropyMap[b as string] - entropyMap[a as string]);
    ...
    const wordCountMap = Promise.all(
        stringColumns.map(
            async (key) => [key, await wordCount(data.map((row) => row[key]) as any[])] as [keyof T, WordCountSummary],
        ),
    ).then((summaries) =>
        summaries.reduce(
    ...
    const metricsMatrix = Promise.all(
        Stream.from(stringKeyCombinations).map(async ([a, b]) => {
            ...
            const [referenceConsistency, reverseConsistency, f1, correlationIndex, transitionEntropy] =
                await Promise.all([
    ...
    const [wordCounts, metrics] = await Promise.all([wordCountMap, metricsMatrix]);

    // Derive the maximum values of each of our metrics for rank normalization
    const maxWordCount = Math.max(...Object.values(wordCounts).map(({ sum }) => sum));
    const maxCooccurrence = Math.max(...metrics.map(({ f1 }) => f1));
    const maxCorrelation = Math.max(...metrics.map(({ correlation }) => correlation));
    const maxTransitionEntropy = Math.max(...metrics.map(({ transitionEntropy }) => transitionEntropy));
    ...
    const dynamicRelationships$ = Promise.all(
        metrics
            .filter(({ f1 }) => maxCooccurrence > 0 && f1 >= maxCooccurrence * lowerF1Bound)
            .map(
                async (row) =>
                    ({
                        ...row,
                        sourceWordCount: (wordCounts[row.source as string] || ({} as WordCountSummary)).sum,
                        targetWordCount: (wordCounts[row.target as string] || ({} as WordCountSummary)).sum,
                        rank:
                            ...
                            (maxTransitionEntropy ? row.transitionEntropy / maxTransitionEntropy : 0),
                        type: 'dynamic',
    ...
    const structuralRelationships$ = Promise.all(
        metrics
            ...
            .map(async (row) => {
                const sourceWordCount = (wordCounts[row.source as string] || ({} as WordCountSummary)).sum,
                    targetWordCount = (wordCounts[row.target as string] || ({} as WordCountSummary)).sum;
                ...
                return {
                    ...
                    rank:
                        3 * (maxTransitionEntropy ? row.transitionEntropy / maxTransitionEntropy : 0) -
        ...
        metrics
            ...
            .map(async (row) => {
                ...
                return {
                    ...
                    targetWordCount: targetWordCount,
                    rank:
                        2 * (maxTransitionEntropy ? row.transitionEntropy / maxTransitionEntropy : 0) -
                        (maxCorrelation ? row.correlation / maxCorrelation : 0) -
                        (maxWordCount ? sourceWordCount / maxWordCount : 0) -
                        (maxWordCount ? targetWordCount / maxWordCount : 0),
                    type: 'other',
    ...
    const [dynamicRelationships, structuralRelationships, otherRelationships] = await Promise.all([
    ...
    const suggestions = {
        ...
        relations: [...dynamicRelationships, ...structuralRelationships, ...otherRelationships]
