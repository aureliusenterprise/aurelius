...
export class FlaskService {
    ...
    public parseDataset(file: File): Observable<ExtractorDatasetEntry[]> {
        ...
        if (file === null || file === undefined) {
            throw new Error('Required parameter file was null or undefined when calling parseDataset.');
        }

        formParams.append('file', file, file.name);

        return this.http.authorize().post<ExtractorDatasetEntry[]>(path, formParams);
    }
}
