...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore
@dataclass
class Process(ProcessDefaultsBase, ProcessBase, ToAtlasConvertible[GenericProcess]):
    def convert_to_atlas(self) -> GenericProcess:
        ...
        attributes = GenericProcessAttributes(
            description=self.definition, name=self.name, qualified_name=self.qualified_name
        )

        if bool(self.process_owner):
            attributes.process_owner = [
                ObjectId(
                    type_name="m4i_person", unique_attributes=M4IAttributes(qualified_name=self.process_owner)
                )
            ]
        # END IF
        ...
        if bool(self.system):
            attributes.system = [
                ObjectId(type_name="m4i_system", unique_attributes=M4IAttributes(qualified_name=self.system))
            ]
        # END IF

        if bool(self.inputs):
            attributes.inputs = [
                ObjectId(type_name="m4i_dataset", unique_attributes=M4IAttributes(qualified_name=input_name))
        ...
        if bool(self.outputs):
            attributes.outputs = [
                ObjectId(type_name="m4i_dataset", unique_attributes=M4IAttributes(qualified_name=output_name))
