...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore
@dataclass
class Person(PersonDefaultsBase, PersonBase, ToAtlasConvertible[AtlasPerson]):
    def convert_to_atlas(self) -> AtlasPerson:
        """
        Returns a corresponding Atlas `AtlasPerson` instance.
        """

        attributes = AtlasPersonAttributes(email=self.email, name=self.name, qualified_name=self.email)
        ...
        entity = AtlasPerson(attributes=attributes)
