...
class CollectionBase(BaseObject):
    ...
    def _qualified_name(self):
        ...
        return get_qualified_name(self.name, prefix=self.system)
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore
@dataclass
class Collection(CollectionDefaultsBase, CollectionBase, ToAtlasConvertible[BusinessCollection]):
    def convert_to_atlas(self) -> BusinessCollection:
        ...
        system = ObjectId(type_name="m4i_system", unique_attributes=unique_attributes)

        attributes = BusinessCollectionAttributes(
            definition=self.definition, name=self.name, qualified_name=self.qualified_name, systems=[system]
        )
        ...
        entity = BusinessCollection(attributes=attributes)
