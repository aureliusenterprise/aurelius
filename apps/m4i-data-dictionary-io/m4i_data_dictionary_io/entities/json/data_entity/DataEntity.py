...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore
@dataclass
class DataEntity(DataEntityDefaultsBase, DataEntityBase, ToAtlasConvertible[BusinessDataEntity]):
    def convert_to_atlas(self) -> BusinessDataEntity:
        """
        Returns a corresponding Atlas `BusinessDataEntity` instance.
        """

        data_domain_unique_attributes = M4IAttributes(qualified_name=self.data_domain)

        data_domain = ObjectId(type_name="m4i_data_domain", unique_attributes=data_domain_unique_attributes)
        ...
        if self.has_parent:
            unique_attributes = M4IAttributes(qualified_name=self.parent_entity or "")

            parent_entity = ObjectId(type_name="m4i_data_entity", unique_attributes=unique_attributes)

            attributes.parent_entity = [parent_entity]
            if self.domain_link != "Yes":
                attributes.data_domain = None  # type: ignore
        # END IF
        ...
        entity = BusinessDataEntity(attributes=attributes)
    ...
    def _qualified_name(self):
        """
        Returns the qualified name of the entity based on its parent `data_domain`,
        its `parent_entity` and its `name`.
        """

        return get_qualified_name(
            self.name,
            prefix=self.parent_entity if self.has_parent else self.data_domain,  # type: ignore
        )
