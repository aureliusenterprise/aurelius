...
def process_transform(data: DataFrame) -> DataFrame:
    data["inputs"] = data[["input_1", "input_2", "input_3", "input_4"]].apply(  # type: ignore
        lambda row: [unpack_dataset_qualified_name(val) for val in row if val != ""], axis=1
    )
    data["outputs"] = data[["output_1", "output_2", "output_3", "output_4"]].apply(  # type: ignore
        lambda row: [unpack_dataset_qualified_name(val) for val in row if val != ""], axis=1
    )
    data = data.drop(  # type: ignore
        columns=["input_1", "input_2", "input_3", "input_4", "output_1", "output_2", "output_3", "output_4"]
    )
