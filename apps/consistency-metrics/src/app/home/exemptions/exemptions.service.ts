...
@Injectable()
export class ExemptionsService extends BasicStore<ExemptionsStoreContext> implements OnDestroy {
    private loadBranches$: Subject<Project> = new Subject<Project>();
    private loadVersions$: Subject<[Project, Branch]> = new Subject<[Project, Branch]>();
    private loadExemptions$: Subject<[Project, Branch, ModelProvenance]> = new Subject<
    ...
    private init() {
        ...
        this.loadVersions$
            .pipe(
                switchMap(([project, branch]) => this.handleLoadVersions(branch, project)),
                untilDestroyed(this),
            )
            .subscribe();
        ...
        this.loadExemptions$
            .pipe(
                switchMap(([project, branch, version]) => this.handleLoadExemptions(project, branch, version)),
                untilDestroyed(this),
            )
            .subscribe();
    }
    ...
    private async handleLoadExemptions(project: Project, branch: Branch, version: ModelProvenance) {
