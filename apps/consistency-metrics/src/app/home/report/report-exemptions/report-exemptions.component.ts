...
import {
    ...
    AddExemptionModalFormContext,
} from '../add-exemption-modal/add-exemption-modal.component';
...
export class ReportExemptionsComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.reportService
            ...
            .pipe(
                map((dataset) =>
                    this.metric ? this.extendTableConfig(this.metric['data'][dataset].config.data) : null,
                ),
            )
            .subscribe((extendedTableConfig) => (this.exemptionsTableConfig = extendedTableConfig));
    }
    ...
    editExemption(exemption: any) {
        const subject: AddExemptionModalFormContext = {
            ...
            scope: {
                type: exemption.version ? 'version' : exemption.branch === '*' ? 'project' : 'branch',
            } as any,
