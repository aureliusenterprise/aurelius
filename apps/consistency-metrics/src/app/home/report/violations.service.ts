...
export class ViolationsService extends BasicStore<ViolationsStoreContext> {
        ...
        combineLatest([
            ...
            .pipe(
                switchMap(([datasetName, violationsPerConcept, exemptionsPerConcept]) =>
                    this.handleFilterVolations(datasetName, violationsPerConcept, exemptionsPerConcept),
                ),
            )
            .subscribe();

        combineLatest([this.reportService.select('dataset'), this.select('filteredViolations')])
            .pipe(
    ...
    private async handleCreateBucketMapping(datasetName: string, violations: Dictionary<any>[]) {
        const bucketMapping: Dictionary<string> = {};

        const { color_column: colorColumn, id_column: idColumn } = await this.reportService.get([
            'metric',
            'data',
            datasetName,
            'config',
    ...
    private async handleFilterVolations(
        ...
        const dataset = await this.reportService.get(['metric', 'data', datasetName]);

        let filteredViolations = dataset.data;

        if (dataset.config.id_column) {
            const violationsPerNonExemptedConcept = omit(violationsPerConcept, Object.keys(exemptionsPerConcept));
            filteredViolations = flatten(Object.values(violationsPerNonExemptedConcept));
        }
    ...
    private async handleIndexViolations(datasetName: string) {
        ...
        const violationsPerConcept =
            dataset && dataset.config.id_column ? groupBy(dataset.data, dataset.config.id_column) : {};
