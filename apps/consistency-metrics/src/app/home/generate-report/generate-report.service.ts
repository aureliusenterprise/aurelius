...
@Injectable()
export class GenerateReportService extends BasicStore<GenerateReportStoreContext> {
    ...
    get versions() {
        return this.select('versionsByTimestamp').pipe(
            map((versions) => orderBy(Object.values(versions), ['start_date'], ['desc'])),
        );
    }

    private init() {
        // Whenever a branch is selected, load the provenance for that branch
        this.loadVersions$.pipe(switchMap(([project, branch]) => this.handleLoadVersions(branch, project))).subscribe();
    }
