...
export class ProjectComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.compareModalService.onModalActivated
            ...
            .subscribe(([branchName, version]) => this.compareModal.activateAndPrefill(branchName, version));

        this.compareModal.submission.pipe(untilDestroyed(this)).subscribe((queryParams) => {
    ...
    async toggleFavorite() {
        const [project, isFavorite] = await Promise.all([
            ...
            this.favoriteProjectsService.isCurrentProjectFavorite.pipe(first()).toPromise(),
        ]);
