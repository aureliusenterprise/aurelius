...
import { compareModels, ModelCompareDifference, ModelCompareDifferenceType } from '@models4insight/repository';
...
export class ModelCompareService extends BasicStore<CompareModelsStoreContext> {
    private readonly compareModels$ = new Subject<[string, string, number?, number?]>();
    ...
    private init() {
        this.compareModels$
            .pipe(
                exhaustMap(([baseBranchName, otherBranchName, baseVersion, otherVersion]) =>
                    this.handleCompareModels(baseBranchName, otherBranchName, baseVersion, otherVersion),
                ),
                untilDestroyed(this),
            )
            .subscribe();
        ...
        this.select('differences')
            .pipe(
                switchMap((differences) => this.handleCreateDifferenceMapping(differences)),
                untilDestroyed(this),
            )
            .subscribe();
    }

    compareModels(baseBranchName: string, otherBranchName: string, baseVersion?: number, otherVersion?: number) {
        this.compareModels$.next([baseBranchName, otherBranchName, baseVersion, otherVersion]);
    }
    ...
    private async handleCompareModels(
        ...
        const { model, differences } = await compareModels(project, baseBranchName, otherBranchName, {
            baseVersion,
            otherVersion,
        }).toPromise();
    ...
    private async handleCreateDifferenceMapping(differences: ModelCompareDifference[]) {
