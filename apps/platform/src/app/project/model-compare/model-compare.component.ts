...
export class ModelCompareComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.bucketMapping$ = this.modelCompareService.select('differencePerConceptId');

        this.isComparingModels$ = this.modelCompareService.select('isComparingModels');

        this.selectedView$ = this.route.queryParamMap.pipe(map((params) => params.get('view')));
    }
    ...
    goBack() {
        if (this.previousRoute) {
            ...
            const queryParams = this.previousRoute ? getQueryParametersFromUrl(this.previousRoute) : {};
