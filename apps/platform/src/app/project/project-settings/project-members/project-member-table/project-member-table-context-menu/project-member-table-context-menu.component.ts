...
export class ProjectMemberTableContextMenuComponent implements OnInit {
    ...
    ngOnInit() {
        this.username$ = this.authenticationService.select(['credentials', 'username']);
    }

    private handleCheckDeletePermission() {
        const isOwner$ = this.projectPermissionService.checkPermission(PermissionLevel.OWNER);

        const isMaintainer$ = this.projectPermissionService.checkPermission(PermissionLevel.MAINTAINER);

        const userPermissions$ = this.projectMemberService.selectMemberPermissions(this.user);

        const userIsNotOwner$ = combineLatest([isMaintainer$, userPermissions$]).pipe(
            map(([isMaintainer, userPermissions]) => isMaintainer && userPermissions > PermissionLevel.OWNER),
        );
