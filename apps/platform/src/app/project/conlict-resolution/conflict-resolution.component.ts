...
export class ConflictResolutionComponent implements OnInit {
    ...
    ngOnInit() {
        this.conflictSets$ = this.projectService
            .select('projectId')
            .pipe(switchMap((projectId) => this.conflictsService.selectConflictSetsForProject(projectId)));

        this.currentContext$ = this.conflictResolutionService.select('currentContext', { includeFalsy: true });
    }
