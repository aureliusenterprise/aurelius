...
@Injectable()
export class ConflictResolutionService extends BasicStore<ConflictResolutionStoreContext> {
    ...
    private init() {
            ...
            .pipe(
                ...
                filter(
                    ([, context, template, count]) =>
                        ...
                        context &&
                        (template !== ConflictResolutionTemplate.MANUAL || count === context.conflictSet.difResult.cnt),
                ),
                exhaustMap(([, context, template]) => this.handleResolveConflicts(context, template)),
            )
            .subscribe();

        // Whenever the apply selected template event fires, set the applied template
        this.applySelectedTemplate$.pipe(withLatestFrom(this.select('selectedTemplate'))).subscribe(([, template]) =>
            this.update({
        ...
        this.select('appliedTemplate')
            .pipe(withLatestFrom(this.select('conflictList'), this.select('currentContext')))
        ...
        combineLatest([this.select('currentOffset'), this.select('currentContext')])
            .pipe(
                concatMap(([offset, context]) =>
                    from(this.handleGetBatch(offset, context)).pipe(map((batch) => [batch, offset])),
                ),
        ...
        this.select('conflictList')
            .pipe(map((conflicts) => conflicts.filter((conflict) => !!conflict.resolution).length))
        ...
        this.select('conflictList')
            .pipe(
                withLatestFrom(this.select('currentContext'), this.select('selectedTemplate')),
                filter(([, , template]) => template !== ConflictResolutionTemplate.MANUAL),
            )
        ...
        if (saveContext.operation === 'add') {
            if (saveContext.side.source === 'left') {
                this.conflictsService.update({
                    ...
                    payload: Object.assign(context, {
                        ...
                        addListRight: new Set(Array.from(context.addListRight).filter((item) => item !== id)),
                        deleteListLeft: new Set(Array.from(context.deleteListLeft).filter((item) => item !== id)),
                        deleteListRight: new Set(Array.from(context.deleteListRight).filter((item) => item !== id)),
                    }),
                });
            } else {
                this.conflictsService.update({
                    ...
                    payload: Object.assign(context, {
                        ...
                        addListLeft: new Set(Array.from(context.addListLeft).filter((item) => item !== id)),
                        deleteListLeft: new Set(Array.from(context.deleteListLeft).filter((item) => item !== id)),
                        deleteListRight: new Set(Array.from(context.deleteListRight).filter((item) => item !== id)),
                    }),
                });
            ...
            if (saveContext.side.source === 'left') {
                this.conflictsService.update({
                    ...
                    path: ['conflictSetsByTaskId', context.taskId],
                    payload: Object.assign(context, {
                        deleteListLeft: new Set([...Array.from(context.deleteListLeft), id]),
                        addListRight: new Set(Array.from(context.addListRight).filter((item) => item !== id)),
                        addListLeft: new Set(Array.from(context.addListLeft).filter((item) => item !== id)),
                        deleteListRight: new Set(Array.from(context.deleteListRight).filter((item) => item !== id)),
                    }),
                ...
                this.conflictsService.update({
                    ...
                    path: ['conflictSetsByTaskId', context.taskId],
                    payload: Object.assign(context, {
                        deleteListRight: new Set([...Array.from(context.deleteListRight), id]),
                        addListRight: new Set(Array.from(context.addListRight).filter((item) => item !== id)),
                        addListLeft: new Set(Array.from(context.addListLeft).filter((item) => item !== id)),
                        deleteListLeft: new Set(Array.from(context.deleteListLeft).filter((item) => item !== id)),
                    }),
        ...
        this.select('conflictList')
            ...
            .subscribe((conflicts) =>
                this.update({
                    ...
                    payload: {
                        conflictList: [
                            ...conflicts.filter((conflict) => conflict !== saveContext.conflict),
    ...
    private async handleGetBatch(offset: number, context: ConflictSetContext): Promise<ConflictMap> {
        ...
        if (conflicts.difResult && conflicts.difResult.conflictList && conflicts.difResult.conflictList.length === 0) {
            this.update({
    ...
    private applyRepositoryOnlyTemplate(conflicts: Conflict[], context: ConflictSetContext) {
        ...
        const deleteListLeft = new Set<string>(
            conflicts
                .filter((conflict: Conflict) => conflict.left && conflict.leftChange === ConflictChangeTypeEnum.DELETED)
                .map((conflict: Conflict) => conflict.left.id),
        );
    ...
    private applyUploadOnlyTemplate(conflicts: Conflict[], context: ConflictSetContext) {
        const addListLeft = new Set<string>();
        ...
        const deleteListRight = new Set<string>(
            conflicts
                .filter(
                    (conflict: Conflict) => conflict.right && conflict.rightChange === ConflictChangeTypeEnum.DELETED,
                )
                .map((conflict: Conflict) => conflict.right.id),
        );
    ...
    private applyUnionRepositoryTemplate(conflicts: Conflict[], context: ConflictSetContext) {
        const addListLeft = new Set<string>(
            conflicts
                .filter(
                    (conflict: Conflict) =>
    ...
    private applyUnionUploadTemplate(conflicts: Conflict[], context: ConflictSetContext) {
        const addListLeft = new Set<string>(
            conflicts
                .filter(
                    (conflict: Conflict) =>
    ...
    private async handleResolveConflicts(
        ...
        const resolutionStatus = conflictResolution.pipe(
            switchMap((status: ModelCommit) => monitorStatus(context.project.project, status.taskId)),
            tap((status) => {
        ...
        const task = this.taskManager.createTask([
            {
                description: 'Resolving conflicts with the previous version of the model',
