        ...
        if (side !== undefined) {
            if (side.source === 'left') {
                this.save.emit({
                    ...
                    operation: this.conflict.leftChange === ConflictChangeTypeEnum.DELETED ? 'delete' : 'add',
                    side: side,
                });
            } else {
                this.save.emit({
                    conflict: this.conflict,
                    operation: this.conflict.rightChange === ConflictChangeTypeEnum.DELETED ? 'delete' : 'add',
                    side: side,
                });
            }
        }
