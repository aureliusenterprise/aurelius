...
const branchNamesDiffer = (control: UntypedFormGroup) => {
    ...
    return !!fromBranch && !!toBranch && fromBranch.name === toBranch.name ? { branchNamesMatch: true } : null;
};
...
export class BranchMergeComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.branchMergeService
            ...
            .pipe(
                ...
                switchMap((fromBranch) => this.branchesService.getBranchByName(fromBranch)),
    ...
    startMerge() {
        ...
        if (this.branchMergeForm.valid) {
            ...
            this.branchMergeService.moveBranches(fromBranch.name, toBranch.name, comment);
        }
    }
