...
export class BranchMergeService extends BasicStore<BranchMergeStoreContext> {
    private readonly moveBranches$: Subject<[string, string, string]> = new Subject<[string, string, string]>();
    ...
    private init() {
        ...
        this.select('branches')
            .pipe(map((branches: BranchSummary[]) => branches.map((branch: BranchSummary) => branch._id).sort()))
    ...
    private async handleBranchMove(fromBranch: string, toBranch: string, comment: string) {
        ...
        const mergeOperation = mergeBranch(project.project, fromBranch, toBranch, username, { comment: comment }).pipe(
            shareReplay(),
        );
        ...
        const checkForConflicts = mergeStatus.pipe(
            ...
            tap(([status, commit]) =>
                this.conflictsService.registerConflictsForTask(status, project, fromBranch, toBranch, commit.taskId, {
                    comment: comment,
                }),
            ),
            shareReplay(),
