...
export class ProjectHomeComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.heatmapData$ = this.projectHomeService.select(['heatmapContext', 'data']);
        this.heatmapDomain$ = this.projectHomeService.select(['heatmapContext', 'domain']);
        this.heatmapLegend$ = this.projectHomeService.select(['heatmapContext', 'legend']);
        this.heatmapRange$ = this.projectHomeService.select(['heatmapContext', 'range']);
        this.heatmapStart$ = this.projectHomeService.select(['heatmapContext', 'start']);
        this.heatmapSubdomain$ = this.projectHomeService.select(['heatmapContext', 'subdomain']);
    ...
    retrieveModel(
        ...
        this.retrieveModelService.retrieveModel(provenance.branch, contentType, provenance.start_date);
    }
    ...
    @ViewChild(CalendarHeatmapComponent, { static: false }) set calendarHeatmap(
        ...
        if (calendarHeatmap) {
            calendarHeatmap.clicked.pipe(untilDestroyed(calendarHeatmap)).subscribe((date) => {
