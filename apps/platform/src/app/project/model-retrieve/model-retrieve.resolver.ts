...
import { BranchSummary, getBranchesSummary, getProvenance, ModelProvenance } from '@models4insight/repository';
...
export class ModelRetrieveResolver implements Resolve<ModelRetrieveParams> {
    ...
    async resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<ModelRetrieveParams> {
        const { project } = await this.projectService.getCurrentProject();

        const filter: FilterContext = {
            pageIndex:
                Number.parseInt(route.queryParamMap.get('modelsPageIndex'), 10) || MODEL_RETRIEVE_DEFAULT_PAGE_INDEX,
