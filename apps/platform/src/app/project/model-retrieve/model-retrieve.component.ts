...
export class ModelRetrieveComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.latestOnly$ = this.modelRetrieveService.select(['filter', 'latestOnly']);
        ...
        this.currentPageIndex$ = this.modelRetrieveService.select(['filter', 'pageIndex']).pipe(shareReplay());
    }
    ...
    setUntilDate(timestamp: number) {
        const endTime = timestamp === null ? null : timestamp + (60 * 60 * 24 * 1000 - 1);
        this.router.navigate(['retrieve'], {
            relativeTo: this.route.parent.parent,
