...
export interface ProvenanceTableContextMenuContext {
    project: Project;
    provenance: ModelProvenance;
}

@Component({
    selector: 'models4insight-provenance-table-context-menu',
    templateUrl: 'provenance-table-context-menu.component.html',
    styleUrls: ['provenance-table-context-menu.component.scss'],
})
export class ProvenanceTableContextMenuComponent {
    provenance: ModelProvenance;
    ...
    private createContextMenuItems(provenance: ModelProvenance): ContextMenuItems {
        return [
            [
                {
                    click: () => this.retrieveModel(provenance.branch, provenance.start_date),
                    hasPermission: () => this.checkPermission(provenance, PermissionLevel.MODEL_USER),
                    icon: faDownload,
            ...
            [
                {
                    click: () => this.exploreModel(provenance.branch, provenance.start_date),
                    hasPermission: () => this.checkPermission(provenance, PermissionLevel.BUSINESS_USER),
                    icon: faGlobe,
                    iconModifier: 'has-text-success',
                    title: 'Explore',
                },
                {
                    click: () => this.compareModalService.activateModal(provenance.branch, provenance.start_date),
                    hasPermission: () => this.checkPermission(provenance, PermissionLevel.BUSINESS_USER),
                    icon: faEquals,
                    iconModifier: 'has-text-success',
                    title: 'Compare',
                },
                {
                    ...
                    hasPermission: () => this.checkPermission(provenance, PermissionLevel.CONTRIBUTOR),
                    icon: faArrowRight,
                    iconModifier: 'has-text-success',
                    title: 'Move from',
                },
            ...
            [
                {
                    click: () => this.openConsistencyMetricsInNewTab(provenance.branch, provenance.start_date),
                    hasPermission: () => this.checkPermission(provenance, PermissionLevel.BUSINESS_USER),
                    icon: faChartPie,
    ...
    private checkPermission(provenance: ModelProvenance, permissionLevel: PermissionLevel): Observable<boolean> {
        return combineLatest([
            this.projectPermissionService.checkPermission(permissionLevel),
            this.branchPermissionService.checkPermission(provenance.branch, permissionLevel),
        ]).pipe(map(([projectPermission, branchPermission]) => projectPermission && branchPermission));
    }
    ...
    private async openConsistencyMetricsInNewTab(branchName: string, version?: number) {
