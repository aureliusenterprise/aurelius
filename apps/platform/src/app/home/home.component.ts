...
import {
    ...
    UserInfoService,
} from '@models4insight/services/user-info';
...
export class HomeComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.skipWelcome$ = this.userInfoService.select(['userInfo', 'skip_welcome']);
    }
    ...
    set favoriteProjectsTable(table: SortableTableComponent<Project>) {
        if (table) {
            table.rowClicked.pipe(untilDestroyed(table)).subscribe((project) => this.navigateToProject(project.id));
        }
    }
