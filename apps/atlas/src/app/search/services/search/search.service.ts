...
export const $DEFAULT_QUERY = new InjectionToken<AppSearchQuery<AppSearchDocument>>('DEFAULT_QUERY');

export interface SearchState<T extends AppSearchDocument, P extends Partial<T> = T> {
    readonly queryObject?: AppSearchQuery<T, P>;
}

@Injectable()
export class SearchService<T extends AppSearchDocument, P extends Partial<T> = T> extends BasicStore<
    SearchState<T, P>
> {
