...
@Injectable()
export class FilterService<T extends AppSearchDocument, P extends Partial<T> = T> extends BasicStore<
    SearchFilterContext<T>
> {
    filters$: Observable<SearchFilters<T>>;

    constructor(private readonly searchResultsService: AppSearchResultsService<T, P>) {
        super();

        this.filters$ = this.select('filters');

        this.searchResultsService.facets$.pipe(untilDestroyed(this)).subscribe((facets) => this.updateFilters(facets));
    }
    ...
    private updateFilters(facets: AppSearchResultsFacets<T>) {
        function formatEntry<K extends keyof T>(key: K, facet: AppSearchResultsFacet<T, T[K]>) {
            return [key, facet.data] as const;
        }

        const entries = Object.entries(facets) as [keyof T, AppSearchResultsFacet<T, T[keyof T]>[]][];
