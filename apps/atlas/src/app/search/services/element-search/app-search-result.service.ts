...
@Injectable()
export class AppSearchResultService<T extends AppSearchDocument, P extends Partial<T> = T>
    implements AppSearchDocumentProvider<P>
{
    ...
    constructor(private readonly searchResultsService: AppSearchResultsService<T, P>) {
        this.document$ = this.searchResultsService.results$.pipe(map((results) => results[0]));
