...
> extends BasicStore<AppSearchResultsStoreContext> {
    ...
    constructor(
        ...
        this.firstPage$ = combineLatest([this.searchService.queryObject$, this.pageSize$]).pipe(
            switchMap(([queryObject, pageSize]) => this.getFirstPage(queryObject, pageSize)),
            shareReplay({ refCount: true }),
        );

        this.facets$ = this.firstPage$.pipe(map((searchContext) => searchContext.facets));

        this.meta$ = this.firstPage$.pipe(map((searchContext) => searchContext.meta));

        this.totalResults$ = this.meta$.pipe(map((meta) => meta.page.total_results));
        ...
        const currentPage$ = currentIndex$.pipe(
            ...
            switchMap(([currentPage, queryObject, pageSize]) => this.getPage(currentPage, queryObject, pageSize)),
        );
        ...
        this.allResultsLoaded$ = combineLatest([this.results$, this.meta$, this.select('allResultsLoaded')]).pipe(
            map(([results, meta, manuallyLoaded]) => manuallyLoaded || results.length === meta.page.total_results),
        );
    }
    ...
    async loadAllPages(): Promise<AppSearchResult<P>[]> {
        const firstPage = await firstValueFrom(this.firstPage$.pipe(take(1), timeout(10000)));

        const totalPages = firstPage.meta.page.total_pages;
        const queryObject = await firstValueFrom(this.searchService.queryObject$.pipe(take(1), timeout(5000)));
