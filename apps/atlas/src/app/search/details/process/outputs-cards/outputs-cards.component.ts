...
import { AppSearchQuery, AtlasEntityDef, AtlasEntitySearchObject } from '@models4insight/atlas/api';
...
export class OutputsCardsSearchService extends EntityDetailsCardsSearchService {
    constructor(private readonly entityDetailsService: EntityDetailsService) {
        ...
        this.entityDetailsService
            .select(['entityDetails', 'entity', 'relationshipAttributes', 'outputs'], {
                includeFalsy: true,
            })
            .pipe(
                defaultIfFalsy([]),
    ...
    private createQueryObject(outputs: AtlasEntityDef[]): AppSearchQuery<AtlasEntitySearchObject, EntitySearchObject> {
        return {
            query: '',
            facets: {
