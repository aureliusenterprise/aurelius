...
import { faArrowAltCircleDown, faFilter } from '@fortawesome/free-solid-svg-icons';
import { AppSearchDocument, AppSearchResult, ElasticSearchResult } from '@models4insight/atlas/api';
...
export class DetailsCardsListComponent<T extends AppSearchDocument = AppSearchDocument> implements OnDestroy {
    ...
    @Input() placeholder = 'search.details.components.detailsCardsList.placeholder';
    @Input() sortingOptions: string[] = [];
    @Input() title: string;

    get searchBarContext(): SimpleSearchInputContext {
        return {
    ...
    constructor(
        ...
        ]).pipe(map(([isFilterActive, showAllDescendants]) => isFilterActive || showAllDescendants));

        this.isLoadingResults$ = this.searchResultsService.isLoadingPage$;
        this.totalResults$ = this.searchResultsService.totalResults$;
        this.query$ = this.searchService.queryObject$.pipe(map((queryObject) => queryObject.query));
    }
