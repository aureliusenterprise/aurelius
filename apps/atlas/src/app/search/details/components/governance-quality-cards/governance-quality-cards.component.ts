...
import { AppSearchQuery, GovQualitySearchObject } from '@models4insight/atlas/api';
...
export class GovernanceQualityCardsSearchService extends DetailsCardsSearchService<GovQualitySearchObject> {
    ...
    private createQueryObject(entityId: string): AppSearchQuery<GovQualitySearchObject> {
        return {
            query: '',
            facets: {
...
const sortingOptions: string[] = ['name', 'dataqualityruledimension', 'compliant', 'dataqualitytype'];
