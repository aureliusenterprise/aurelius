...
import { AppSearchQuery, AtlasEntitySearchObject } from '@models4insight/atlas/api';
...
import { EntitySearchObject, ENTITY_SEARCH_FIELDS } from '../../../services/search/entity-search.service';
...
@Injectable()
export class CollectionsCardsSearchService extends EntityDetailsCardsSearchService {
    constructor(
        ...
        combineLatest([this.entityDetailsService.select('entityId'), this.showDescendantsService.showDescendants$])
            .pipe(
                map(([entityId, showDescendants]) => this.createQueryObject(entityId, showDescendants)),
                untilDestroyed(this),
            )
    ...
    private createQueryObject(
        ...
        return {
            ...
            filters: {
                all: [
                    { supertypenames: ['m4i_collection'] },
                    {
                        [showDescendants ? 'derivedsystemguid' : 'parentguid']: [entityId],
                    },
                ],
            },
        };
...
@Component({
    selector: 'models4insight-collections-cards',
    ...
    providers: [
        ...
        CollectionsCardsSearchService,
        {
            provide: DetailsCardsSearchService,
            useExisting: CollectionsCardsSearchService,
        },
        { provide: SearchService, useExisting: CollectionsCardsSearchService },
        ShowDescendantsService,
    ],
})
export class CollectionsCardsComponent {
