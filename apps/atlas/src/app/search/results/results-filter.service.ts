...
import { FilterService, SearchFilters } from '../services/filter/filter.service';
...
function sortFilterFields(filters: SearchFilters<AtlasEntitySearchObject>) {
    return Object.fromEntries(FIELD_ORDER.filter((key) => key in filters).map((key) => [key, filters[key]]));
}

@Injectable()
export class ResultsFilterService extends FilterService<AtlasEntitySearchObject> {
    constructor(searchResultsService: AppSearchResultsService<AtlasEntitySearchObject>) {
        super(searchResultsService);

        this.filters$ = this.filters$.pipe(map(sortFilterFields), shareReplay({ refCount: true }));
    }
}
