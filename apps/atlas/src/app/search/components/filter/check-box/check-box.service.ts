...
export interface CheckBoxStoreContext<T extends AppSearchDocument, K extends keyof T> {
    readonly facet?: K;
    readonly value?: AppSearchFilterValue<T, K>;
    readonly count?: number;
}

function indexFiltersByKey<T extends AppSearchDocument>(filters: AppSearchFilters<T>) {
    if (!filters?.all) {
...
@Injectable()
export class CheckBoxService<T extends AppSearchDocument, K extends keyof T> extends BasicStore<
    CheckBoxStoreContext<T, K>
> {
    ...
    private readonly updateFilterSelection$: Subject<boolean> = new Subject<boolean>();
    ...
    constructor(private readonly searchService: SearchService<T>) {
        ...
        this.isSelected$ = combineLatest([
            ...
            this.facet$,
            this.value$,
        ]).pipe(map(([filters, facet, value]) => this.isSelected(filters, facet, value)));
    ...
    private isSelected(filters: AppSearchFilters<T>, facet: K, value: AppSearchFilterValue<T, K>) {
        if (!filters?.all) return false;

        const filtersByKey = indexFiltersByKey(filters);

        return facet in filtersByKey && filtersByKey[facet as string].includes(value);
    }
