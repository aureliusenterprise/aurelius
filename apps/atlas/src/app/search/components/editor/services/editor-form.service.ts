...
export const EDITOR_FORM_FACTORY = new InjectionToken<EditorFormFactory>('EDITOR_FORM_FACTORY');
...
export const EDITOR_MERGE_STRATEGY = new InjectionToken<EditorMergeStrategy>('EDITOR_MERGE_STRATEGY');

export type EditorUpdateStrategy = (entityDetails: AtlasEntityWithEXTInformation, form: UntypedFormGroup) => void;

export const EDITOR_UPDATE_STRATEGY = new InjectionToken<EditorMergeStrategy>('EDITOR_UPDATE_STRATEGY');
...
export class EditorFormService implements OnDestroy {
    ...
    private init() {
        this.formData$ = combineLatest([
            ...
            .subscribe(([entityDetails, _]) => this.updateStrategy(entityDetails, this.form));
    ...
    @ManagedTask('search.components.editor.submit', { isQuiet: true })
    private async handleSubmitForm() {
        ...
        const updatedEntity = this.mergeStrategy(cloneDeep(entityDetails), this.form);
