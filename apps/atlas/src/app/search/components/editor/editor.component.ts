...
import { EntityAuditEvent, GovQualityApi, GovQualityApiClient } from '@models4insight/atlas/api';
...
export class EditorComponent implements OnDestroy {
    ...
    constructor(
        ...
        this.isDeletingEntity$ = this.entityDeleteService.select('isDeletingEntity');

        this.isUpdatingEntity$ = this.entityUpdateService.select('isUpdatingEntity');

        this.entityDeleteService.entityDeleted.pipe(untilDestroyed(this)).subscribe(() => this.onEntityDeleted());
