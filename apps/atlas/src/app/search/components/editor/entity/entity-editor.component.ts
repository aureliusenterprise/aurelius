...
function updateEntityEditorForm(entityDetails: AtlasEntityWithEXTInformation, form: UntypedFormGroup) {
    ...
    const childEntities: UntypedFormArray = relationshipAttributes.get('childEntity') as UntypedFormArray;

    const dataAttributes: UntypedFormArray = relationshipAttributes.get('attributes') as UntypedFormArray;

    const dataDomain: UntypedFormArray = relationshipAttributes.get('dataDomain') as UntypedFormArray;

    const parentEntity: UntypedFormArray = relationshipAttributes.get('parentEntity') as UntypedFormArray;
    ...
    childEntities.clear();
    entityDetails.entity.relationshipAttributes.childEntity?.forEach((dataEntity) =>
        childEntities.push(new UntypedFormControl(dataEntity)),
    );
    ...
    parentEntity.clear();
    entityDetails.entity.relationshipAttributes.parentEntity?.forEach((dataEntity) =>
        parentEntity.push(new UntypedFormControl(dataEntity)),
    );
...
export class EntityEditorComponent {
    ...
    ngOnInit(): void {
        const parents = this.parentEntities as unknown as UntypedFormArray;
        const domains = this.dataDomains as unknown as UntypedFormArray;

        if (parents && domains) {
            this.subscriptions.push(
                parents.valueChanges.subscribe(() => {
                    if (parents.length > 0 && domains.length > 0) {
                        domains.clear();
                    }
                }),
            );
            ...
            this.subscriptions.push(
                domains.valueChanges.subscribe(() => {
                    if (domains.length > 0 && parents.length > 0) {
                        parents.clear();
                    }
                }),
            );
    ...
    get childEntities() {
        return this.editorFormService.form.get('relationshipAttributes.childEntity');
    }
    ...
    get parentEntities() {
        return this.editorFormService.form.get('relationshipAttributes.parentEntity');
    }
