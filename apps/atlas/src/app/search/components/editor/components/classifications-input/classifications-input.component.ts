...
function filterClassifications(
    ...
    const applicableDefs = defs.filter((def) => def.entityTypes?.find((entityType) => entityType === typeName));

    const defsNotSelected = applicableDefs.filter(
        (def) => !selected.find((classification) => classification.typeName === def.name),
    );
...
export class ClassificationsInputComponent implements OnInit {
    ...
    ngOnInit() {
        const typeName$ = this.entityDetailsService.entityDetails$.pipe(map((entity) => entity.typeName));
        ...
        ]).pipe(map(([defs, selected, typeName, query]) => filterClassifications(defs, selected, typeName, query)));
    }

    async addTag(typeName: string) {
        const entityId = await this.entityDetailsService.get(['entityDetails', 'entity', 'guid']);
