...
function updateDomainEditorForm(entityDetails: AtlasEntityWithEXTInformation, form: UntypedFormGroup) {
    ...
    const dataEntities: UntypedFormArray = relationshipAttributes.get('dataEntity') as UntypedFormArray;

    const domainLeads: UntypedFormArray = relationshipAttributes.get('domainLead') as UntypedFormArray;
    ...
    dataEntities.clear();
    entityDetails.entity.relationshipAttributes.dataEntity?.forEach((dataEntity) =>
        dataEntities.push(new UntypedFormControl(dataEntity)),
    );

    domainLeads.clear();
    entityDetails.entity.relationshipAttributes.domainLead?.forEach((domainLead) =>
        domainLeads.push(new UntypedFormControl(domainLead)),
    );
}
