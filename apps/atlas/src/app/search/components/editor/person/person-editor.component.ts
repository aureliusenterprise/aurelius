...
function createPersonEditorForm(): UntypedFormGroup {
    const email = new UntypedFormControl(null, [Validators.required, Validators.email]),
...
function updateProcessEditorForm(entityDetails: AtlasEntityWithEXTInformation, form: UntypedFormGroup) {
    ...
    const businessOwnerAttribute = relationshipAttributes.get('businessOwnerAttribute') as UntypedFormArray;

    const businessOwnerEntity = relationshipAttributes.get('businessOwnerEntity') as UntypedFormArray;

    const domainLead = relationshipAttributes.get('domainLead') as UntypedFormArray;

    const stewardAttribute = relationshipAttributes.get('stewardAttribute') as UntypedFormArray;

    const stewardEntity = relationshipAttributes.get('stewardEntity') as UntypedFormArray;

    attributes.patchValue(entityDetails.entity.attributes);

    businessOwnerAttribute.clear();
    entityDetails.entity.relationshipAttributes.businessOwnerAttribute?.forEach((attribute) =>
        businessOwnerAttribute.push(new UntypedFormControl(attribute)),
    );

    businessOwnerEntity.clear();
    entityDetails.entity.relationshipAttributes.businessOwnerEntity?.forEach((entity) =>
        businessOwnerEntity.push(new UntypedFormControl(entity)),
    );
    ...
    entityDetails.entity.relationshipAttributes.stewardAttribute?.forEach((attribute) =>
        stewardAttribute.push(new UntypedFormControl(attribute)),
    );
...
export class PersonEditorComponent {
    ...
    get businessOwnerAttributes() {
        return this.editorFormService.form.get('relationshipAttributes.businessOwnerAttribute');
    }

    get businessOwnerEntities() {
        return this.editorFormService.form.get('relationshipAttributes.businessOwnerEntity');
    }
    ...
    get stewardAttributes() {
        return this.editorFormService.form.get('relationshipAttributes.stewardAttribute');
    }

    get stewardEntities() {
        return this.editorFormService.form.get('relationshipAttributes.stewardEntity');
    }
