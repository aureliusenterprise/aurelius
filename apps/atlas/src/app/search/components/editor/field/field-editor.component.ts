...
function updateFieldEditorForm(entityDetails: AtlasEntityWithEXTInformation, form: UntypedFormGroup) {
    ...
    const childField: UntypedFormArray = relationshipAttributes.get('childField') as UntypedFormArray;

    const dataAttributes: UntypedFormArray = relationshipAttributes.get('attributes') as UntypedFormArray;

    const datasets: UntypedFormArray = relationshipAttributes.get('datasets') as UntypedFormArray;

    const parentField: UntypedFormArray = relationshipAttributes.get('parentField') as UntypedFormArray;
...
export class FieldEditorComponent {
    ...
    ngOnInit(): void {
        const parents = this.parentFields as unknown as UntypedFormArray;
        const datasets = this.datasets as unknown as UntypedFormArray;

        if (parents && datasets) {
            this.subscriptions.push(
                parents.valueChanges.subscribe(() => {
                    if (parents.length > 0 && datasets.length > 0) {
                        datasets.clear();
                    }
                }),
            );
            ...
            this.subscriptions.push(
                datasets.valueChanges.subscribe(() => {
                    if (datasets.length > 0 && parents.length > 0) {
                        parents.clear();
                    }
                }),
            );
    ...
    get parentFields() {
        return this.editorFormService.form.get('relationshipAttributes.parentField');
    }
