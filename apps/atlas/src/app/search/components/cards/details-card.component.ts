...
import { AppSearchDocument, AppSearchResult, EntityElementWithEXTInfo } from '@models4insight/atlas/api';
...
export class DetailsCardComponent implements OnDestroy {
    ...
    constructor(
        ...
        this.name$ = this.searchResultService.document$.pipe(map((document) => document.name?.raw));

        this.intersectionObserver.onIntersection
            .pipe(switchMapTo(this.searchResultService.document$), untilDestroyed(this))
            .subscribe((document) => (this.entityDetailsService.entityId = document.guid?.raw));
    }
