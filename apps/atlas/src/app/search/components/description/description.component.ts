...
export class DescriptionComponent {
    description$: Observable<string> = this.entityDetailsService
    ...
    private selectDescriptionField(typeName: string): Observable<string> {
        if (typeName === 'm4i_gov_data_quality' || typeName === 'm4i_data_quality') {
            return this.entityDetailsService.select(['entityDetails', 'entity', 'attributes', 'ruleDescription'], {
                includeFalsy: true,
            });
        } else {
            return this.entityDetailsService.select(['entityDetails', 'entity', 'attributes', 'definition'], {
                includeFalsy: true,
            });
        }
    }
