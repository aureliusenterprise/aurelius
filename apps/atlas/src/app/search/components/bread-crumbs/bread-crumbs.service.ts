...
function fmtBreadcrumb([guid, name, typeName]: [string, string, string]): Breadcrumb {
    return {
        guid,
        name,
        typeName,
...
export class BreadCrumbsService extends BasicStore<BreadCrumbsStoreContext> {
    ...
    private handleUpdateBreadcrumbs(searchResult: AppSearchResult<AtlasEntitySearchObject>) {
