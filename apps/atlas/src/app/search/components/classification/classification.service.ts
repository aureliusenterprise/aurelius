...
import { ClassificationDef, ClassificationDefAPIService } from '@models4insight/atlas/api';
...
export class ClassificationService extends BasicStore<ClassificationStoreContext> {
    constructor(private readonly classificationDefApiService: ClassificationDefAPIService) {
        super();
        this.init();
    }
    ...
    private init() {
        this.select('classificationName')
            .pipe(
                switchMap((classificationName) => this.handleRetrieveClassificationDef(classificationName)),
                untilDestroyed(this),
            )
            .subscribe();
    }
