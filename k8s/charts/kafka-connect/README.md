...
```console
helm install --set kafka.bootstrapServers="PLAINTEXT://lolling-chinchilla-cp-kafka-headless:9092",cp-schema-registry.url="lolling-chinchilla-cp-schema-registry:8081" cp-helm-charts/charts/kafka-connect
```
...
1. A [Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/) `kissing-macaw-kafka-connect
` which contains 1 Kafka Connect [Pod](https://kubernetes.io/docs/concepts/workloads/pods/pod-overview/): `kissing-macaw-kafka-connect-6c77b8f5fd-cqlzq`.
2. A [Service](https://kubernetes.io/docs/concepts/services-networking/service/) `kissing-macaw-kafka-connect`
   for clients to connect to Kafka Connect REST endpoint.
...
The configuration parameters in this section control the resources requested and utilized by the `kafka-connect`
chart.
...
#### FileConfigProvider Configuration

The chart supports Confluent's FileConfigProvider for dynamic configuration using environment variables prefixed
with `CONFIG_`. These can be used to reference secrets or config files at runtime.

| Parameter                                                      | Description                                                     | Default            |
| -------------------------------------------------------------- | --------------------------------------------------------------- | ------------------ |
| `configurationOverrides.CONNECT_CONFIG_PROVIDERS`              | Enables file-based config provider (e.g., `"file"`)             | Not set by default |
| `configurationOverrides.CONNECT_CONFIG_PROVIDERS_FILE_CLASS`   | FileConfigProvider class name                                   | Not set by default |
| `configurationOverrides.CONNECT_CONFIG_STORAGE_CLEANUP_POLICY` | Cleanup policy for config storage topic (`compact` or `delete`) | `compact`          |
| `configurationOverrides.CONNECT_OFFSET_STORAGE_CLEANUP_POLICY` | Cleanup policy for offset storage topic                         | `compact`          |
| `configurationOverrides.CONNECT_STATUS_STORAGE_CLEANUP_POLICY` | Cleanup policy for status storage topic                         | `compact`          |

#### CONFIG\_\* Environment Variables
...
The chart creates a dedicated secret (`kafka-connect-secrets`) for storing sensitive configuration values used
by `FileConfigProvider`. The following keys are supported:

| Key                           | Description               |
| ----------------------------- | ------------------------- |
| `config-elastic-password`     | Elasticsearch password    |
| `config-elastic-key-password` | SSL keystore key password |
| `config-keystore-password`    | SSL keystore password     |
| `config-truststore-password`  | SSL truststore password   |
