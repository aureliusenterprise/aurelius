...
def test__health_endpoint_returns_200(client: FlaskClient):
...
@responses.activate
def test__proxy_forwards_get_request(client: FlaskClient):
...
@responses.activate
def test__proxy_forwards_post_request_with_body(client: FlaskClient):
...
@responses.activate
def test__proxy_replaces_authorization_header(client: FlaskClient):
...
@responses.activate
def test__proxy_preserves_other_headers(client: FlaskClient):
...
@responses.activate
def test__proxy_returns_error_status_from_backend(client: FlaskClient):
...
def test__build_target_url_with_path(settings: Settings):
...
def test__build_target_url_with_trailing_slash_in_base(settings: Settings):
...
def test__build_target_url_with_empty_path(settings: Settings):
    """Empty path returns just the base URL."""
    url = build_target_url(settings, "")
    assert url == "http://app-search.test"


def test__build_target_url_with_leading_slash_in_path(settings: Settings):
    """Leading slash in path is handled correctly."""
    url = build_target_url(settings, "/api/as/v1/search")
    assert url == "http://app-search.test/api/as/v1/search"


def test__unauthorized_without_auth_header(client: FlaskClient):
...
@responses.activate
def test__proxy_with_root_path(client: FlaskClient):
