...
def parse_dataset(access_token=None):
    """
    Parses a source data file into a set of JSON records.
    Supports XLS, XLSX, CSV and JSON.
...
supported_layouts = LayoutRegistry(
    [ArchimateHierarchicalLayout, ArchimateProcessLayout]
)
...
def extract(access_token=None):
    ...
    # Since the analytics library is agnostic of most layout implementations,
    # check whether the given layout matches any of the ones we know.
    # If the layout is supported, replace the name with the layout instance
    # and add the view to the list we will process further.
    ...
    element_definitions = [ElementDefinition(**el) for el in rules.get("elements", [])]
    ...
    relationship_definitions = [
        RelationshipDefinition(**fmt_relation(rel))
        for rel in rules.get("relations", [])
    ]
    ...
    def fmt_view(view):
        def fmt_view_element(element):
            element_ref = next(
                (
                    element_ref
                    for element_ref in element_definitions
                    if element_ref["id"] == element["rule"]
                ),
                None,
            )
        ...
        def fmt_view_relation(relation):
            relation_ref = next(
                (
                    relation_ref
                    for relation_ref in relationship_definitions
                    if relation_ref["id"] == relation["rule"]
                ),
                None,
            )
    ...
    view_definitions = [ViewDefinition(**fmt_view(view)) for view in views_with_layout]

    # Generate the nodes from the data based on the given definitions
    ...
    if not nodes_dataframe.empty:
        ...
        if nodes_dataframe.shape[0] > 1:
            nodes_dataframe.drop_duplicates(subset=["id"], keep="last", inplace=True)
        # END IF
    ...
    if not edges_dataframe.empty:
        ...
        if edges_dataframe.shape[0] > 1:
            edges_dataframe.drop_duplicates(subset=["id"], keep="last", inplace=True)
        # END IF
    ...
    generated_views = ExtractorLanguagePrimitives.parse_view(
        dataframe,
        model,
        view_definitions,  # type: ignore[reportGeneralTypeIssues]
        "Model Extractor API",
        False,  # type: ignore[reportGeneralTypeIssues]
    )

    # Return the generated model and metadata
    result = {
        "metadata": list(extracted_elements["metadata"])
        + list(extracted_relations["metadata"])
        + list(generated_views["metadata"]),
