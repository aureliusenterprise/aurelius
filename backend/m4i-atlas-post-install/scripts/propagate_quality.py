...
def parse_args():
    ...
    parser.add_argument(
        "--atlas-dev", default=Path("data/atlas-dev.json"), type=Path, help="Path to atlas-dev JSON dump"
    )
    parser.add_argument(
        "--quality",
...
def update_quality_field(
    quality_document: MutableMapping[str, Any], entity_document: MutableMapping[str, Any]
):
    ...
    entity_document[f"dqscore_{dimension}"] = (
        entity_document[f"dqscoresum_{dimension}"] / entity_document[f"dqscorecnt_{dimension}"]
    )
    entity_document[f"qualityguid_{dimension}"].append(quality_document["qualityguid"])


def update_quality_fields(
    quality_documents: List[MutableMapping[str, Any]], atlas_dev_index: MutableMapping[str, Any]
):
...
def update_quality_scores(
    downstream_document: MutableMapping[str, Any], upstream_document: MutableMapping[str, Any], dimension: str
):
    upstream_document[f"dqscoresum_{dimension}"] += downstream_document[f"dqscoresum_{dimension}"]
    upstream_document[f"dqscorecnt_{dimension}"] += downstream_document[f"dqscorecnt_{dimension}"]

    if upstream_document[f"dqscorecnt_{dimension}"] == 0:
        return

    upstream_document[f"dqscore_{dimension}"] = (
        upstream_document[f"dqscoresum_{dimension}"] / upstream_document[f"dqscorecnt_{dimension}"]
    )


def update_quality_scores_per_dimension(
    downstream_document: MutableMapping[str, Any], upstream_document: MutableMapping[str, Any]
):
    for dimension in DIMENSIONS:
        update_quality_scores(
            downstream_document=downstream_document, upstream_document=upstream_document, dimension=dimension
        )


def update_quality_attributes(atlas_dev_index: MutableMapping[str, Any]):
    for document in atlas_dev_index.values():
        if document["typename"] == "m4i_field":
            for attribute_guid in document["deriveddataattributeguid"]:
                update_quality_scores_per_dimension(
                    downstream_document=document, upstream_document=atlas_dev_index[attribute_guid]
                )


def propagate_quality(atlas_dev_index: MutableMapping[str, Any]):
    for document in atlas_dev_index.values():
        if document["typename"] in ["m4i_data_attribute", "m4i_field"]:
            ...
            for breadcrumb_guid in document["breadcrumbguid"]:
                ...
                update_quality_scores_per_dimension(
                    downstream_document=downstream_document, upstream_document=upstream_document
                )
...
def update_overall_score(document: MutableMapping[str, Any]):
    ...
    document["dqscore_overall"] = document["dqscoresum_overall"] / document["dqscorecnt_overall"]
...
def main():
    ...
    index_all_documents(
        engine_name="atlas-dev", app_search_client=app_search_client, documents=list(atlas_dev_index.values())
    )
    print("Updated atlas-dev")
    index_all_documents(
        engine_name="atlas-dev-quality", app_search_client=app_search_client, documents=quality_documents
    )
