...
from m4i_metrics.physical import (
...
from m4i_metrics.process import (
...
from m4i_metrics.structural import (
...
from m4i_metrics.textual import ConceptLabelFormattingMetric, LabelAndConceptDuplicationMetric, TextualMetric
...
async def generate_metric(metric_key: str, model_options: Dict[str, Any]) -> Dict[str, Any]:
    """
    Calculates the metric associated with the given `metric_key` for the model
    which matches the given `model_options`.
    ...
    if issubclass(metric, MetricCategory):
        return await _calculate_metric_category(metric_category=metric, model_options=model_options)
    else:
        return await _get_metric(
            metric_name=metric_key, access_token=model_options["access_token"], format_data=False
        )
    # END IF
...
async def calculate_metric(metric_key: str, model_options: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if issubclass(metric, MetricCategory):
        raise ValueError("The given metric key belongs to a metric category")
    # END IF
    ...
    with ThreadPoolExecutor() as pool:
        ...
        model = await loop.run_in_executor(pool, get_model)
    # END WITH
    ...
    return {"type": "table", "data": _format_metric(metric_result)}


# END calculate_metric


async def _calculate_metric_category(metric_category: type, model_options: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    metric_names: List[str] = [metric.id for metric in metric_category.metrics]

    # Resolve the metrics and exemptions asynchronously
    def resolve_metrics():
        return asyncio.gather(
            *(_get_metric(metric_name, model_options["access_token"]) for metric_name in metric_names)
        )
    ...
    submetrics, exemptions = await asyncio.gather(resolve_metrics(), resolve_exemptions())

    summary: Dict[str, Any] = metric_category.summarize(submetrics, exemptions)

    # Return a JSON serializable result
    return {
        "type": "chart",
        "chart": json_item(
            model=_format_chart(chart=metric_category.create_graph(summary["Summary"]["data"]))
        ),
...
def _format_chart(chart: Figure) -> Figure:
    ...
    chart.sizing_mode = "stretch_both"  # type: ignore[assignment]
    chart.toolbar.logo = None  # type: ignore[assignment]
    return chart
...
def _format_metric(metric_result: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    def fmt_data(data: DataFrame) -> List[Dict[str, Any]]:
        return (
            data.reset_index().reset_index().replace({nan: None}).to_dict(orient="records")  # type: ignore[return-value]
        )

    # END fmt_data

    result: Dict[str, Any] = {
        key: {**value, "config": value["config"].to_dict(), "data": fmt_data(value["data"])}
...
async def _get_metric_exemptions(
    project_name: str, branch_name: str, version: int, metric_name: str, access_token: str
):
    """
    This function retrieves the exemptions for the project with the given name,
    branch with the given name, version and metric.

    Because the exemptions api expects a project id and branch id, this function
    retrieves the project and branch matching the given names first.
    ...
    project: Any = PlatformApi.retrieve_project(project_name, access_token=access_token)

    branches: Sequence[Any] = PlatformApi.get_branches(project.id, access_token=access_token)

    branch_with_name: Optional[Any] = next(
    ...
    if branch_with_name:
        ...
        with ThreadPoolExecutor() as pool:
            result = await loop.run_in_executor(pool, get_exemptions)
        # END WITH
    # END IF
...
async def _get_metric(metric_name: str, access_token: str, format_data: bool = True) -> Dict[str, Any]:
    """
    Retrieves the meric result for the metric with the given name. Makes a
    local HTTP request to potentially hit the cache.

    :return: The metric result for the metric with the given name as if it was
        calculated directly by the metric
    :rtype: dict
    """

    # This builds the request url based on path the original call, ensuring
    # the request always makes it to this server
    ...
    with ThreadPoolExecutor() as pool:
        ...
        def get_metric():
            return ApiUtils.get(request_path, params=request_args, access_token=access_token)

        # ENd get_metric

        metric: Any = await loop.run_in_executor(pool, get_metric)
    # END WITH

    # Retrieve the metric from the API.
    metric_json: Dict[str, Any] = loads(metric)  # type: ignore[arg-type]

    if format_data:
        # Parse the retrieved JSON and format the data of each dataset as a DataFrame
        metric_json = {
            dataset_name: {**dataset, "data": DataFrame(dataset["data"])}
