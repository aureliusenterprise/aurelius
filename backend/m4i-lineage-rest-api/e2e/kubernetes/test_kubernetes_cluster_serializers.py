...
def test_m4i_kubernetes_cluster_model_no_kubernetes_environment(client, request_to_make):
    ...
    t = client.post(
        path, headers={"Content-Type": "application/json"}, json=request_no_kubernetes_environment
    )
    assert t.status_code == 400
    t_json = t.get_json()
    assert t_json["message"] == "Input payload validation failed"
    assert t_json["errors"] == {"kubernetesEnvironment": "'kubernetesEnvironment' is a required property"}
...
def test_m4i_kubernetes_cluster_model_empty_kubernetes_namespace(
    ...
    t = client.post(
        path, headers={"Content-Type": "application/json"}, json=request_empty_kubernetes_namespace
    )
