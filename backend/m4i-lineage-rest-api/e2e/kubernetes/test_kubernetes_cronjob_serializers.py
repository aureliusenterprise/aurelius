...
def test_m4i_kubernetes_cronjob_model_no_kubernetes_cluster(client, request_to_make):
    ...
    assert t_json["errors"] == {"kubernetesNamespace": "'kubernetesNamespace' is a required property"}


# END test_m4i_kubernetes_cronjob_model_no_kubernetes_namespace


def test_m4i_kubernetes_cronjob_model_no_kubernetes_pod(client, request_to_make):
    ...
    t_json = t.get_json()
    assert t_json["message"] == "Input payload validation failed"
    assert t_json["errors"] == {"kubernetesPod": "'kubernetesPod' is a required property"}


# END test_m4i_kubernetes_cronjob_model_no_kubernetes_pod


def test_m4i_kubernetes_cronjob_model_empty_kubernetes_pod(client, request_to_make, check_made, cleanup):
...
# END test_m4i_kubernetes_cronjob_model_empty_kubernetes_pod


def test_m4i_kubernetes_cronjob_model_no_tags(client, request_to_make, check_made, cleanup):
...
# END test_m4i_kubernetes_cronjob_model_no_tags


def test_m4i_kubernetes_cronjob_model_no_schedule(client, request_to_make, check_made, cleanup):
    request_no_schedule = request_to_make.copy()
    request_no_schedule.pop("schedule")

    t = client.post(path, headers={"Content-Type": "application/json"}, json=request_no_schedule)
    assert t.status_code == 200  # Does it claim to have been made?
    assert t.json == {"CREATE": 1, "UPDATE": 2, "DELETE": 0}
    guid = asyncio.run(check_made(entity_qn=entity_qn, entity_type=entity_type))
    asyncio.run(cleanup(guid=guid, entity_qn=entity_qn, entity_type=entity_type))
...
# END test_m4i_kubernetes_cronjob_model_no_schedule
