...
@pytest.fixture
def request_to_make():
    return {"name": "testing_m4i_elastic_cluster", "replica_count": 1, "shard_count": 2}
...
def test_elastic_cluster_get(client):
    t = client.get(path)
    assert t.status_code == 200
    # END test_elastic_cluster_get


def test_elastic_cluster_post(client, request_to_make, check_made, cleanup):
...
# END test_elastic_cluster_post


def test_elastic_cluster_post_potency_double(client, request_to_make, check_made, cleanup):
    t = client.post(path, headers={"Content-Type": "application/json"}, json=request_to_make)
    assert t.status_code == 200
    assert t.json == {"CREATE": 1, "UPDATE": 0, "DELETE": 0}
    guid = asyncio.run(check_made(entity_qn=entity_qn, entity_type=entity_type))
...
# END test_elastic_cluster_post_potency_double


def test_elastic_cluster_post_potency_diff(client, request_to_make, check_made, cleanup):
    request_diff = request_to_make.copy()
    request_diff["replica_count"] = 5
...
# END test_elastic_cluster_post_potency_diff
