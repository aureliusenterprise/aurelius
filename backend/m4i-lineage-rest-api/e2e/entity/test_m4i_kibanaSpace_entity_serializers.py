...
def test_m4i_kibana_space_model_no_avatar_color(client, request_to_make):
    ...
    t_json = t.get_json()
    assert t_json["message"] == "Input payload validation failed"
    assert t_json["errors"] == {"avatar_color": "'avatar_color' is a required property"}
...
def test_m4i_kibana_space_model_no_avatar_initials(client, request_to_make):
    ...
    t_json = t.get_json()
    assert t_json["message"] == "Input payload validation failed"
    assert t_json["errors"] == {"avatar_initials": "'avatar_initials' is a required property"}
...
def test_m4i_kibana_space_model_no_elastic_cluster(client, request_to_make):
    ...
    t_json = t.get_json()
    assert t_json["message"] == "Input payload validation failed"
    assert t_json["errors"] == {"elastic_cluster": "'elastic_cluster' is a required property"}
