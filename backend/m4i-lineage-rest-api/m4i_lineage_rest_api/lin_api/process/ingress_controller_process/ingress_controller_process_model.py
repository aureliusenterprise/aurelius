...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class IngressControllerProcess(IngressControllerProcessDefaultsBase, IngressControllerProcessBase):
    def convert_to_atlas(self) -> CoreIngressControllerProcess:
        ...
        if bool(self.ingress_object):
            ...
            for so in self.ingress_object:
                ingress_object_unique_attributes = M4IAttributes(qualified_name=so)
        ...
        cluster_unique_attributes = M4IAttributes(qualified_name=self.cluster)
        cluster.append(
            ObjectId(type_name="m4i_kubernetes_cluster", unique_attributes=cluster_unique_attributes)
        )

        relationship_attributes = IngressControllerProcessRelationshipAttributes(cluster=cluster)
