...
from m4i_atlas_core.entities.atlas.processes.ApiOperationProcess import (
    ApiOperationProcess as CoreApiOperationProcess,
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class ApiOperationProcess(ApiOperationProcessDefaultsBase, ApiOperationProcessBase):
    def convert_to_atlas(self) -> CoreApiOperationProcess:
        ...
        if bool(self.microservice):
            ...
            microservice_unique_attributes = M4IAttributes(qualified_name=self.microservice)
            microservice.append(
                ObjectId(
        ...
        entity = CoreApiOperationProcess(attributes=attributes)
