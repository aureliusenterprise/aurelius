...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class KubernetesServiceProcess(KubernetesServiceProcessDefaultsBase, KubernetesServiceProcessBase):
    def convert_to_atlas(self) -> CoreKubernetesServiceProcess:
        ...
        if bool(self.ingress_object):
            ...
            ingress_object_unique_attributes = M4IAttributes(qualified_name=self.ingress_object)
            ingress_object.append(
                ObjectId(
                    type_name="m4i_ingress_object_process", unique_attributes=ingress_object_unique_attributes
                )
            )
        ...
        if bool(self.microservice):
            ...
            for so in self.microservice:
                microservice_unique_attributes = M4IAttributes(qualified_name=so)
                microservice.append(
                    ObjectId(
                        type_name="m4i_microservice_process", unique_attributes=microservice_unique_attributes
                    )
                )
            # END FOR
        ...
        namespace_unique_attributes = M4IAttributes(qualified_name=self.namespace)
        namespace.append(
            ObjectId(type_name="m4i_kubernetes_namespace", unique_attributes=namespace_unique_attributes)
        )

        relationship_attributes = KubernetesServiceProcessRelationshipAttributes(namespace=namespace)

        entity = CoreKubernetesServiceProcess(
            attributes=attributes, relationship_attributes=relationship_attributes
        )
