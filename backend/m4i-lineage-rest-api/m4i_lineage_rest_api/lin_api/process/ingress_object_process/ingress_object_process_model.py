...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class IngressObjectProcess(IngressObjectProcessDefaultsBase, IngressObjectProcessBase):
    def convert_to_atlas(self) -> CoreIngressObjectProcess:
        ...
        if bool(self.kubernetes_service):
            ...
            for so in self.kubernetes_service:
                kubernetes_service_unique_attributes = M4IAttributes(qualified_name=so)
                kubernetes_service.append(
                    ObjectId(
        ...
        if bool(self.ingress_controller):
            ...
            ingress_controller_unique_attributes = M4IAttributes(qualified_name=self.ingress_controller)
            ingress_controller.append(
                ObjectId(
        ...
        relationship_attributes = IngressObjectProcessRelationshipAttributes(namespace=namespace)
