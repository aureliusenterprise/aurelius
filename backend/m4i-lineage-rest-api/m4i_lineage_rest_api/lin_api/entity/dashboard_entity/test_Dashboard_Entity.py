...
def request_to_make():
    return {
        ...
        "child_dataset": ["test_m4i_visualization"],
    }
...
def test_dashboard_get(client):
    ...
    # END test_dashboard_get


def test_dashboard_post(client, request_to_make, check_made, cleanup):
...
# END test_dashboard_post


def test_dashboard_post_potency_double(client, request_to_make, check_made, cleanup):
    t = client.post(path, headers={"Content-Type": "application/json"}, json=request_to_make)
    assert t.status_code == 200
    assert t.json == {"CREATE": 1, "UPDATE": 3, "DELETE": 0}
    guid = asyncio.run(check_made(entity_qn=entity_qn, entity_type=entity_type))
...
# END test_dashboard_post_potency_double


def test_dashboard_post_potency_diff(client, request_to_make, check_made, cleanup):
...
# END test_dashboard_post_potency_diff
