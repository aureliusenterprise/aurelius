...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class Dashboard(DashboardDefaultsBase, DashboardBase):
    def convert_to_atlas(self) -> CoreDashboard:
        ...
        if bool(self.kibana_space):
            unique_attributes = M4IAttributes(qualified_name=self.kibana_space)

            kibana_space = ObjectId(type_name="m4i_kibana_space", unique_attributes=unique_attributes)

            attributes.kibana_space = [kibana_space]
        # END IF

        entity = CoreDashboard(attributes=attributes)
