...
from m4i_atlas_core import (
    ObjectId,
    M4IAttributes,
    ConfluentEnvironment as CoreConfluentEnvironment,
...
class ConfluentEnvironmentBase(DataClassJsonMixin):
    ...
    def _qualified_name(self):
        ...
        return get_qualified_name(self.name, prefix=self.confluent_cloud or "")
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class ConfluentEnvironment(ConfluentEnvironmentDefaultsBase, ConfluentEnvironmentBase):
    def convert_to_atlas(self) -> CoreConfluentEnvironment:
        ...
        if self.confluent_cloud is None:
            raise ValueError("confluent_cloud is required for ConfluentEnvironment")
        confluent_cloud_unique_attributes = M4IAttributes(qualified_name=self.confluent_cloud)
        confluent_cloud.append(
            ObjectId(type_name="m4i_confluent_cloud", unique_attributes=confluent_cloud_unique_attributes)
        )
        ...
        entity = CoreConfluentEnvironment(attributes=attributes)
