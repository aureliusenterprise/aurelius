...
from m4i_atlas_core import ObjectId, M4IAttributes, KSQL as CoreKSQL, KSQLAttributes as CoreKSQLAttributes
...
class KSQLBase(DataClassJsonMixin):
    ...
    def _qualified_name(self):
        ...
        cluster = getattr(self, "cluster", None) or ""
        return get_qualified_name(self.name, prefix=f"{env}--{cluster}")
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class KSQL(KSQLDefaultsBase, KSQLBase):
    def convert_to_atlas(self) -> CoreKSQL:
        ...
        kafka_topic = []
        referencable_unique_attributes = M4IAttributes(qualified_name=self.kafka_topic)

        kafka_topic.append(
            ObjectId(type_name="m4i_kafka_topic", unique_attributes=referencable_unique_attributes)
        )
        ...
        entity = CoreKSQL(attributes=attributes)
