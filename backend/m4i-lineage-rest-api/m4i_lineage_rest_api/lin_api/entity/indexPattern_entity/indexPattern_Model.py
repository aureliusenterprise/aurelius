...
from m4i_atlas_core import (
    ObjectId,
    M4IAttributes,
    IndexPattern as CoreIndexPattern,
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class IndexPattern(IndexPatternDefaultsBase, IndexPatternBase):
    def convert_to_atlas(self) -> CoreIndexPattern:
        ...
        if bool(self.parent_dataset):
            ...
            for parent_dataset in self.parent_dataset:
                ...
                parent_dataset_list.append(
                    ObjectId(type_name="m4i_visualization", unique_attributes=unique_attributes)
                )
        ...
        attributes = CoreIndexPatternAttributes(
            ...
            name=self.name,
            creator=attr_creator if attr_creator else [],
            qualified_name=self._qualified_name(),
            parent_dataset=attr_parent_dataset if attr_parent_dataset else [],
            description=self.description,
        )

        entity = CoreIndexPattern(attributes=attributes)
