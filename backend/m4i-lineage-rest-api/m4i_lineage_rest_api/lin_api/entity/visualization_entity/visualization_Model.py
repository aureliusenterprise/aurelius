...
from m4i_atlas_core import (
    ObjectId,
    M4IAttributes,
    Visualization as CoreVisualization,
...
class VisualizationBase(DataClassJsonMixin):
    ...
    def _qualified_name(self):
        ...
        return get_qualified_name(self.qualified_name)
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class Visualization(VisualizationDefaultsBase, VisualizationBase):
    def convert_to_atlas(self) -> CoreVisualization:
        ...
        if bool(self.child_dataset):
            ...
            for child_dataset in self.child_dataset:
                unique_attributes = M4IAttributes(qualified_name=child_dataset)
                child_dataset_list.append(
                    ObjectId(type_name="m4i_index_pattern", unique_attributes=unique_attributes)
                )
        ...
        if bool(self.parent_dataset):
            ...
            for parent_dataset in self.parent_dataset:
                unique_attributes = M4IAttributes(qualified_name=parent_dataset)
                parent_dataset_list.append(
                    ObjectId(type_name="m4i_dashboard", unique_attributes=unique_attributes)
                )
        ...
        if bool(self.creator):
            ...
            for creator in self.creator:
                unique_attributes = M4IAttributes(qualified_name=creator)
                creator_list.append(ObjectId(type_name="m4i_person", unique_attributes=unique_attributes))
            attributes.creator = creator_list
        # END IF

        entity = CoreVisualization(attributes=attributes)
