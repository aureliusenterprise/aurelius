...
def request_to_make():
    return {
        ...
        "parentDataset": ["test_m4i_dashboard"],
        "childDataset": ["test_m4i_indexPattern"],
        "type": "a type",
...
def test_m4i_visualization_model_no_updatedAt(client, request_to_make):
    ...
    assert t_json["errors"] == {"updatedAt": "'updatedAt' is a required property"}
...
def test_m4i_visualization_model_no_version(client, request_to_make):
    ...
    t_json = t.get_json()
    assert t_json["message"] == "Input payload validation failed"
    assert t_json["errors"] == {"version": "'version' is a required property"}
...
def test_m4i_visualization_model_no_type(client, request_to_make):
    ...
    t_json = t.get_json()
    assert t_json["message"] == "Input payload validation failed"
    assert t_json["errors"] == {"type": "'type' is a required property"}
...
def test_m4i_visualization_model_no_childDataset(client, request_to_make, check_made, cleanup):
    ...
    t_json = t.get_json()
    assert t_json["message"] == "Input payload validation failed"
    assert t_json["errors"] == {"childDataset": "'childDataset' is a required property"}
