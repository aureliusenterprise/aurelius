...
from m4i_atlas_core import KafkaTopic, KafkaTopicAttributes, M4IAttributes, ObjectId
...
class KafkaTopicApiModelBase(DataClassJsonMixin):
    ...
    def _qualified_name(self):
        ...
        env = getattr(self, "environment", None) or ""
        return f"{env}--{self.cluster}--{self.name}"
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class KafkaTopicApiModel(KafkaTopicApiModelDefaultsBase, KafkaTopicApiModelBase):
    def convert_to_atlas(self):
        """
        Returns a corresponding Atlas `KafkaTopic` instance, along with
        `KafkaField` instances representing the value schema.
        """

        collection_unique_attributes = M4IAttributes(
            qualified_name=f"{self.environment or ''}--{self.cluster}--data"
        )

        collection = [
            ObjectId(type_name="m4i_collection", unique_attributes=collection_unique_attributes, guid="-1")
        ]
        cluster_unique_attributes = M4IAttributes(qualified_name=f"{self.environment or ''}--{self.cluster}")

        cluster = [ObjectId(type_name="m4i_kafka_cluster", unique_attributes=cluster_unique_attributes)]
        ...
        entity = KafkaTopic(attributes=attributes)

        if not isinstance(self.value_schema, str):
            fields = []
            fields_ref = []
            for field in self.value_schema.fields:
                fields_entity, field_refs = field.convert_to_atlas(
                    self._qualified_name(), entity.guid, "", ""
                )
