...
from m4i_atlas_core import (
    ObjectId,
    M4IAttributes,
    ConfluentCloud as CoreConfluentCloud,
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class ConfluentCloud(ConfluentCloudDefaultsBase, ConfluentCloudBase):
    def convert_to_atlas(self) -> CoreConfluentCloud:
        """
        Returns a corresponding Atlas `ConfluentCloud` instance.
        """
        attributes = CoreConfluentCloudAttributes(name=self.name, qualified_name=self._qualified_name())

        if bool(self.confluent_environment):
            confluent_environment: List[ObjectId] = []
            for ce in self.confluent_environment:
                confluent_environment_unique_attributes = M4IAttributes(qualified_name=ce)
                confluent_environment.append(
                    ObjectId(
        ...
        entity = CoreConfluentCloud(attributes=attributes)
