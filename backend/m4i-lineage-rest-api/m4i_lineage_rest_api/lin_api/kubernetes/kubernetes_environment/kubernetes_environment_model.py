...
from m4i_atlas_core.entities.atlas.kubernetes import (
    KubernetesEnvironment as CoreKubernetesEnvironment,
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class KubernetesEnvironment(KubernetesEnvironmentDefaultsBase, KubernetesEnvironmentBase):
    def convert_to_atlas(self) -> CoreKubernetesEnvironment:
        ...
        attributes = CoreKubernetesEnvironmentAttributes(
            definition=self.description, name=self.name, qualified_name=self.qualified_name
        )

        if bool(self.kubernetes_clusters):
            kubernetes_clusters: List[ObjectId] = []
            for kc in self.kubernetes_clusters:
                kubernetes_clusters_unique_attributes = M4IAttributes(qualified_name=kc)
                kubernetes_clusters.append(
                    ObjectId(
        ...
        entity = CoreKubernetesEnvironment(attributes=attributes)
