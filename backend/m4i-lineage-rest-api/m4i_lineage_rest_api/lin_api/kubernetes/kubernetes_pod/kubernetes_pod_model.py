...
from m4i_atlas_core.entities.atlas.kubernetes import (
    KubernetesPod as CoreKubernetesPod,
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class KubernetesPod(KubernetesPodDefaultsBase, KubernetesPodBase):
    def convert_to_atlas(self) -> CoreKubernetesPod:
        ...
        if bool(self.kubernetes_deployment):
            ...
            kubernetes_deployment_unique_attributes = M4IAttributes(qualified_name=self.kubernetes_deployment)
            kubernetes_deployment.append(
                ObjectId(
        ...
        if bool(self.kubernetes_cronjob):
            ...
            kubernetes_cronjob_unique_attributes = M4IAttributes(qualified_name=self.kubernetes_cronjob)
            kubernetes_cronjob.append(
                ObjectId(
                    type_name="m4i_kubernetes_cronjob", unique_attributes=kubernetes_cronjob_unique_attributes
                )
            )
        ...
        if bool(self.microservice):
            ...
            for ms in self.microservice:
                microservice_unique_attributes = M4IAttributes(qualified_name=ms)
                microservice.append(
                    ObjectId(
        ...
        entity = CoreKubernetesPod(attributes=attributes)
