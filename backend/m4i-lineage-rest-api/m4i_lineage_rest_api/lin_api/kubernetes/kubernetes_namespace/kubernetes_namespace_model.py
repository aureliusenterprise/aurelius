...
from m4i_atlas_core.entities.atlas.kubernetes import (
    KubernetesNamespace as CoreKubernetesNamespace,
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class KubernetesNamespace(KubernetesNamespaceDefaultsBase, KubernetesNamespaceBase):
    def convert_to_atlas(self) -> CoreKubernetesNamespace:
        ...
        kubernetes_cluster_unique_attributes = M4IAttributes(qualified_name=self.kubernetes_cluster)
        kubernetes_cluster.append(
            ObjectId(
                type_name="m4i_kubernetes_cluster", unique_attributes=kubernetes_cluster_unique_attributes
            )
        )
        ...
        if bool(self.kubernetes_deployment):
            ...
            for kd in self.kubernetes_deployment:
                kubernetes_deployment_unique_attributes = M4IAttributes(qualified_name=kd)
        ...
        if bool(self.kubernetes_cronjob):
            ...
            for kc in self.kubernetes_cronjob:
                kubernetes_cronjob_unique_attributes = M4IAttributes(qualified_name=kc)
        ...
        entity = CoreKubernetesNamespace(attributes=attributes)
