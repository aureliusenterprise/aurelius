...
Is used to determine the quality of various entities already loaded into
DMP's governance tool - Apache Atlas.
It verifies data loaded against various m4i types
(like m4i_data_domain, m4i_data_entity) on quality measures
like completeness and uniqueness.
...
We can execute `run.py` file.
This will generate 6 files in the output folder of the package:
three each for attributes and relationships.
In addition, generated data is pushed to Elasticsearch indexes.
We can configure pre-fix of indexes by updating
...
Please make a copy of `config.sample.py` and `credentials.sample.py`
and rename the files to `config.py` and `credentials.py` respectively.
...
Data needed to be added to App search and entities are created in dev atlas.
new function `_push_rules_to_atlas` in `Rules` class was added,
