...
import { catchError, concatMap, shareReplay, switchMap, switchMapTo, takeLast, takeUntil, tap } from 'rxjs/operators';
...
export class Task {
    ...
    async addOperation(operation: OperationContext) {
        const operations = await this.managedBy.get(['tasks', this.id, 'operations'], { includeFalsy: true });
    ...
    async getExecutable() {
        ...
        const pipeline = zip(range(0, task.operations.length), from(task.operations)).pipe(
            // Execute each operation sequentially. Ensure each task completes by only taking at most one emission from the stream
            concatMap(([index, operation]) => this.handleOperation(task.id, operation, index)),
            takeLast(1),
        );
        ...
        const tasks = of(task).pipe(tap(onStart), switchMapTo(pipeline), tap(onComplete), catchError(onError));
    ...
    private handleOperation(taskId: string, operation: OperationContext, index: number) {
        this.managedBy.update({
...
export class TaskManagerService extends BasicStore<TaskManagerStoreContext> {
    ...
    createTask(operations: OperationContext[] = [], options: TaskOptions = {}): Task {
        const taskId = uuid();
    ...
    private async handleRemoveCompletedTasks(tasks: TaskContext[]) {
        for (const task of tasks) {
            const clearAfterError = task.currentState === TaskState.ERROR && task.options.clearOnError,
                clearAfterSuccess = task.currentState === TaskState.DONE && task.options.clearOnSuccess;
