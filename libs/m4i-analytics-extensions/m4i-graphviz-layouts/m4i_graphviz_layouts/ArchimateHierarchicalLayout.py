...
class ArchimateHierarchicalLayout(HierarchicalLayout):
    @staticmethod
    def _prepare_model(model):
        """
        Prepares a model for the hierarchical layout visualization by inverting
        all access writes and specialization relations.
        :param ArchimateModel model: the model that should be prepared.
        :return ArchimateModel: A copy of the given model where all access write
            and specialization relationships have been inverted.
        """
        ...
        relations = (
            relation
            for index, relation in working_copy.edges.iterrows()
    ...
    @staticmethod
    def get_coordinates(
        graph, dpi=80, rankdir="BT", nodesep=1, ranksep=2, node_width=0.1, node_height=0.1, **kwargs
    ):
        prepared_model = ArchimateHierarchicalLayout._prepare_model(graph)
        return HierarchicalLayout.get_coordinates(
            prepared_model, dpi, rankdir, nodesep, ranksep, node_width, node_height, **kwargs
        )
    ...
    @staticmethod
    def get_name() -> str:
        return "archimate_hierarchical"

    # END get_name


# END ArchimateHierarchicalLayout
