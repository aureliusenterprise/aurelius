...
from m4i_atlas_core.entities import (
    BusinessDataDomain,
...
async def test__get_entity_by_qualified_name_single_search_result():
    ...
    search_result = SearchResult(
        approximate_count=1,
        entities=[EntityHeader(guid="12345", type_name=type_name)],
...
async def test__get_entity_by_qualified_name_no_search_results():
    ...
    search_result = SearchResult(approximate_count=0, entities=[], query_type=QueryType.ATTRIBUTE)
...
async def test__get_entity_by_qualified_name_multiple_search_results():
    ...
    search_result = SearchResult(
        approximate_count=2,
        entities=[
            EntityHeader(guid="12345", type_name=type_name),
            EntityHeader(guid="23456", type_name=type_name),
        ],
        query_type=QueryType.ATTRIBUTE,
    )
