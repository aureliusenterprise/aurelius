...
data_quality_attributes_def = [
    ...
    AttributeDef(
        ...
        type_name="string",
        description=(
            "Functional description on what the data field needs to "
            "adhere to including the related data fields which can be "
            "used to program into a measurable rule."
        ),
    ...
    AttributeDef(
        ...
        type_name="string",
        description=(
            "The expression that is used by a Python script "
            "to apply the correct functionality for a data quality rule."
        ),
    ...
    AttributeDef(
        ...
        type_name="boolean",
        description=(
            "Classification if a filter is required for a data quality rule "
            "(this is required in case a field contains a content structure)."
        ),
...
end_1_dataQuality_fields = RelationshipEndDef(
    type="m4i_data_quality", name="fields", cardinality=Cardinality.SET
)
end_2_dataQuality_fields = RelationshipEndDef(
    type="m4i_field", name="dataQuality", cardinality=Cardinality.SET
)
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class BusinessDataQualityAttributes(
    BusinessDataQualityAttributesDefaultsBase, BusinessDataQualityAttributesBase
):
    pass
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class BusinessDataQuality(  # type: ignore[reportGeneralTypeIssues]
    BusinessDataQualityDefaultsBase, BusinessDataQualityBase, Entity
):
