...
class TimeZone(DataClassJsonMixin):
    dst_savings: Optional[int] = field(metadata=config(field_name="DSTSavings"))
    id: Optional[str] = field(metadata=config(field_name="ID"))
    available_ids: List[str] = field(default_factory=list, metadata=config(field_name="availableIDs"))
