...
class ObjectIdHasNoReferenceException(Exception):
    """
    This exception is raised whenever an `ObjectId` is declared
    without a `guid` and without any `unique_attributes`.
    """
...
class ObjectId(ObjectIdDefaultsBase, ObjectIdBase):
    ...
    def validate_reference(self):
        ...
        has_unmapped_unique_attribute = (
            self.unique_attributes is not None
            and self.unique_attributes.unmapped_attributes is not None
            and any(self.unique_attributes.unmapped_attributes.values())
        )
        ...
        has_mapped_unique_attribute = False
        if self.unique_attributes is not None:
