...
kubernetes_service_process_attributes_def = [
    AttributeDef(
        name="ingressObject", type_name="array<m4i_ingress_object_process>", cardinality=Cardinality.SET
    ),
    AttributeDef(
...
end_1_knamespace_ksprocess = RelationshipEndDef(type="m4i_kubernetes_namespace", name="service")
end_2_knamespace_ksprocess = RelationshipEndDef(type="m4i_kubernetes_service_process", name="namespace")
...
end_1_ioprocess_ksprocess = RelationshipEndDef(type="m4i_ingress_object_process", name="kubernetesService")
end_2_ioprocess_ksprocess = RelationshipEndDef(type="m4i_kubernetes_service_process", name="ingressObject")
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class KubernetesServiceProcessAttributes(  # type: ignore[reportGeneralTypeIssues]
    GenericProcessAttributes,
...
class KubernetesServiceProcessBase(GenericProcessBase):
    attributes: KubernetesServiceProcessAttributes  # type: ignore[reportIncompatibleMethodOverride]
...
class KubernetesServiceProcessDefaultsBase(GenericProcessDefaultsBase):
    relationship_attributes: Optional[KubernetesServiceProcessRelationshipAttributes] = None  # type: ignore[reportIncompatibleMethodOverride]
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class KubernetesServiceProcess(  # type: ignore[reportGeneralTypeIssues]
    GenericProcess, KubernetesServiceProcessDefaultsBase, KubernetesServiceProcessBase
):
