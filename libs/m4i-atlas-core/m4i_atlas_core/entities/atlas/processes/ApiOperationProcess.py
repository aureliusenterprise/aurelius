...
end_1_msprocess_aoprocess = RelationshipEndDef(type="m4i_microservice_process", name="operation")
end_2_msprocess_aoprocess = RelationshipEndDef(type="m4i_api_operation_process", name="microservice")
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class ApiOperationProcessAttributes(  # type: ignore[reportGeneralTypeIssues]
    GenericProcessAttributes, ApiOperationProcessAttributesDefaultsBase, ApiOperationProcessAttributesBase
):
    pass
...
class ApiOperationProcessBase(GenericProcessBase):
    attributes: ApiOperationProcessAttributes  # type: ignore[reportIncompatibleMethodOverride]
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class ApiOperationProcess(  # type: ignore[reportGeneralTypeIssues]
    GenericProcess, ApiOperationProcessDefaultsBase, ApiOperationProcessBase
):
    ...
    def get_referred_entities(self) -> Iterable[ObjectId]:
        ...
        references = [*super().get_referred_entities(), *self.attributes.microservice]
