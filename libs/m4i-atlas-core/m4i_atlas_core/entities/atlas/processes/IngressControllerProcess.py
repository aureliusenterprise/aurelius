...
end_1_kcluster_icprocess = RelationshipEndDef(type="m4i_kubernetes_cluster", name="ingressController")
end_2_kcluster_icprocess = RelationshipEndDef(type="m4i_ingress_controller_process", name="cluster")
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class IngressControllerProcessAttributes(  # type: ignore[reportGeneralTypeIssues]
    GenericProcessAttributes,
...
class IngressControllerProcessBase(GenericProcessBase):
    attributes: IngressControllerProcessAttributes  # type: ignore[reportIncompatibleMethodOverride]
...
class IngressControllerProcessDefaultsBase(GenericProcessDefaultsBase):
    relationship_attributes: Optional[IngressControllerProcessRelationshipAttributes] = None  # type: ignore[reportIncompatibleMethodOverride]
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class IngressControllerProcess(  # type: ignore[reportGeneralTypeIssues]
    GenericProcess, IngressControllerProcessDefaultsBase, IngressControllerProcessBase
):
    ...
    def get_referred_entities(self) -> Iterable[ObjectId]:
        ...
        references = [
            ...
            *(self.relationship_attributes.cluster if self.relationship_attributes else []),
        ]
