...
confluent_environment_attributes_def = [
    AttributeDef(name="schemaRegistry", type_name="boolean"),
    AttributeDef(name="confluentCloud", type_name="array<m4i_confluent_cloud>", cardinality=Cardinality.SET),
    AttributeDef(name="kafkaClusters", type_name="array<m4i_kafka_cluster>", cardinality=Cardinality.SET),
]
...
end_1_kcluster_cenvironment = RelationshipEndDef(
    type="m4i_confluent_environment", name="kafkaClusters", cardinality=Cardinality.SET
)
end_2_kcluster_cenvironment = RelationshipEndDef(
    type="m4i_kafka_cluster", name="confluentEnvironment", cardinality=Cardinality.SET
)
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class ConfluentEnvironmentAttributes(
    BusinessSystemAttributes, ConfluentEnvironmentAttributesDefaultsBase, ConfluentEnvironmentAttributesBase
):
    pass
...
class ConfluentEnvironmentBase(BusinessSystemBase):
    attributes: ConfluentEnvironmentAttributes  # type: ignore[reportIncompatibleMethodOverride]
...
@dataclass_json(letter_case=LetterCase.CAMEL)  # type: ignore[argument-type]
@dataclass
class ConfluentEnvironment(  # type: ignore[reportGeneralTypeIssues]
    BusinessSystem, ConfluentEnvironmentDefaultsBase, ConfluentEnvironmentBase
):
