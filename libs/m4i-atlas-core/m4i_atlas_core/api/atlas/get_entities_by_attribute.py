...
from ...entities import FilterCriteria, Operator, SearchParameters, SearchResult
...
async def get_entities_by_attribute(
    attribute_name: str, attribute_value: str, type_name: str, access_token: Optional[str] = None
):
    """
    ...
    params = SearchParameters(
        entity_filters=FilterCriteria(
            attribute_name=attribute_name, attribute_value=attribute_value, operator=Operator.EQ
        ),
        type_name=type_name,
    )

    response: str = await atlas_post(path=PATH, body=params.to_json(), access_token=access_token)
