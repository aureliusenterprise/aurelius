...
async def get_entity_by_guid(
    guid: str,
    entity_type: Union[Type[T], str] = Entity,
    ...
    if isinstance(entity_type, str):
        entity_type = get_entity_type_by_type_name(entity_type)  # type: ignore[reportGeneralTypeIssues]
    # END IF
    ...
    if guid.startswith("-"):
        return entity_type(guid=guid, attributes={})  # type: ignore[reportGeneralTypeIssues]
    # END IF
    ...
    return entity_type.from_dict(response["entity"])  # type: ignore[reportGeneralTypeIssues]
