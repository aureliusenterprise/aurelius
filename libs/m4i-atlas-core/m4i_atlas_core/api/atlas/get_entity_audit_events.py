...
@cached()
async def get_entity_audit_events(entity_guid: str, access_token: Optional[str] = None):
    """
    Fetch all audit events for an entity given its GUID.

    Optionally, you can filter on a particular action type,
    limit the number of events returned, or offset the history
    based on a particular event key.
    """

    path = PATH_TEMPLATE.format(entity_guid=entity_guid)

    response: str = await atlas_get(path=path, access_token=access_token)
