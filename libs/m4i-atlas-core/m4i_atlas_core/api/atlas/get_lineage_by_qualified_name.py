...
@cached()
async def get_lineage_by_qualified_name(
    qualified_name: str,
    type_name: str,
    depth: int = 3,
    ...
    Options:
    * You can use `depth` to specify the maximum number of hops
      to traverse the lineage graph. Default is 3.
    * You can use `direction` to specify whether to retrieve input
      lineage, output lineage or both. Default is both.
    ...
    * Raises a `404` error if no lineage is found for the given entity.
    ...
    params = {"depth": depth, "direction": direction.value, "qualifiedName": qualified_name}

    response: str = await atlas_get(path=path, params=params, access_token=access_token)
