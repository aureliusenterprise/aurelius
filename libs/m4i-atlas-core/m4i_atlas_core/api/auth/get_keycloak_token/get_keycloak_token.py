...
def get_keycloak_token(
    ...
    - **keycloak.server.url**: The url of the Keycloak server. Local:
      `http://localhost:8180/auth`. External: `https://domain/auth`
    - **keycloak.client.id**: The name of the Keycloak client.
    - **keycloak.realm.name**: The name of the Keycloak realm.
    - **keycloak.client.secret.key**: The public RS256 key associated with
      the Keycloak realm.
    ...
    :param keycloak KeycloakOpenID: Optional Keycloak instance. If none is given,
        creates one using pre-configured parameters. Default is `None`.
    ...
    if credentials is not None:
        ...
        username, password = store.get_many(
            "keycloak.credentials.username", "keycloak.credentials.password", all_required=True
        )
    # END IF

    token = keycloak.token(username=username, password=password, totp=totp)

    if not isinstance(token, dict):
        raise TypeError(f"Expected token response to be a dict, got {type(token).__name__}")
