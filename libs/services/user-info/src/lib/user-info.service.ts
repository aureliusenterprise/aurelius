...
export class UserInfoService extends BasicStore<UserInfoStoreContext> {
    ...
    private init() {
        const [authenticated, notAuthenticated] = partition(this.authenticationService.isAuthenticated(), identity);
        ...
        this.select('userInfo')
            .pipe(
                pairwise(),
                filter(([old, current]) => old?.userid && current?.userid && old?.userid === current?.userid),
                debounceTime(50),
