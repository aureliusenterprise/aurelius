...
export class FavoriteProjectsService extends BasicStore<FavoriteProjectsStoreContext> {
    ...
    get isCurrentProjectFavorite() {
        ...
        ]).pipe(map(([projectId, favoriteProjects]) => !!favoriteProjects?.includes(projectId)));
    }
    ...
    private async handleAddFavoriteProject(project: Project) {
        ...
        const updatedFavoriteProjects = [
            project?.id,
            ...favoriteProjects.filter((favoriteProject) => favoriteProject !== project?.id),
        ];
    ...
    private async handleRemoveFavoriteProject(project: Project) {
        const favoriteProjects = await this.userInfoService.get(['userInfo', 'favorite_projects']);

        const updatedFavoriteProjects = favoriteProjects.filter((favoriteProject) => favoriteProject !== project?.id);
