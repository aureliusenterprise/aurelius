    ...
    selectConflictSetsForProject(projectId: string) {
        return combineLatest([
            ...
            map(([conflictSetsByTaskId, taskIds]) =>
                taskIds.map((taskId) => conflictSetsByTaskId[taskId]).filter((conflictSet) => !!conflictSet),
            ),
        );
    }

    private groupConflictSetsByProjectId(conflictSetsByTaskId: Dictionary<ConflictSetContext>) {
        const conflictSetsPerProject = Object.entries(conflictSetsByTaskId).reduce(
            (result, [taskId, conflictSet]) => ({
                ...result,
                [conflictSet.project.id]: [...(result[conflictSet.project.id] ?? []), taskId],
            }),
