...
export class ModelService extends BasicStore<ModelStoreContext> {
    ...
    private init() {
        ...
        combineLatest([this.select('branch', { includeFalsy: true }), this.select('version', { includeFalsy: true })])
            .pipe(
                debounceTime(50),
                filter(([branch]) => !!branch),
                switchMap(([branch, version]) => this.handleRetrieveModel(branch, version)),
                untilDestroyed(this),
            )
            .subscribe();
    }
    ...
    private async handleRetrieveModel(branch: string, version: number) {
        ...
        const model = await getModel('archimate3', project, branch, '', 'TRUNK', username, 'json', {
            version,
        }).toPromise();
