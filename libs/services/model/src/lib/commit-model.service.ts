...
export class CommitModelService extends BasicStore<CommitModelStoreContext> {
    ...
    async commitModel(branchId: string, comment: string, model: File, options: CommitOptions = {}) {
    ...
    async commitJsonModel(branchId: string, comment: string, model: string, options: CommitOptions = {}) {
        this.commitModel$.next({
            branchId,
            comment,
            model,
    ...
    private async handleCommitModel(
        ...
        const commitMethods = {
            ...
            json: () =>
                commitJsonModel(project.project, branchName, model as string, username, {
                    comment,
                }),
        };
        ...
        const commitStatus = uploadModel.then((commit) => monitorStatus(project.project, commit.taskId).toPromise());
