...
export class BranchesService extends BasicStore<BranchesStoreContext> {
    ...
    private init() {
        ...
        this.projectService
            .select('projectId')
            .pipe(
                switchMap((projectId) => this.watch(['branchesPerProject', projectId, 'branchesById'])),
    ...
    async getBranchById(branchId: string) {
        ...
        return this.get(['branchesPerProject', projectId, 'branchesById', branchId]);
    }
    ...
    async getBranchByName(branchName: string) {
        const projectId = await this.projectService.get('projectId');
        return this.get(['branchesPerProject', projectId, 'branchesByName', branchName]);
    }
    ...
    selectBranchById(branchId: string) {
        return this.projectService
            .select('projectId')
            .pipe(switchMap((projectId) => this.select(['branchesPerProject', projectId, 'branchesById', branchId])));
    }
    ...
    selectBranchByName(branchName: string) {
        return this.projectService
            .select('projectId')
            .pipe(
                switchMap((projectId) => this.select(['branchesPerProject', projectId, 'branchesByName', branchName])),
            );
    }
    ...
    get branches(): Observable<Branch[]> {
        return this.projectService.select('projectId').pipe(
            switchMap((projectId) => this.select(['branchesPerProject', projectId, 'branchesById'])),
            map(Object.values),
        );
    }
    ...
    private async handleDeleteBranch(branch: Branch) {
        ...
        const deleteBranchResponse = await deleteBranch(projectId, branchId).toPromise();
