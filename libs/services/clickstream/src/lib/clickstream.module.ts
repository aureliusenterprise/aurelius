...
export class ClickstreamModule {
    constructor(clickstreamService: ClickstreamService, @Optional() @SkipSelf() parentModule: ClickstreamModule) {
        if (parentModule) {
    ...
    static forRoot(config: ClickstreamConfig): ModuleWithProviders<ClickstreamModule> {
        return {
