...
export class ProjectMembersService extends BasicStore {
    ...
    private readonly updateProjectMember$ = new Subject<[string, PermissionLevel]>();
    private readonly projectMemberUpdated$ = new Subject<Dictionary<PermissionLevel>>();
    ...
    private init() {
        ...
        this.updateProjectMember$
            .pipe(
                concatMap(([username, permission]) => this.handleUpdateMemberPermissions(username, permission)),
                untilDestroyed(this),
            )
     ...
     */
    addProjectMember(username: string, permission: PermissionLevel = PermissionLevel.BUSINESS_USER) {
    ...
    async getCurrentUserPermissions() {
        const username = await this.authenticationService.get(['credentials', 'username']);
    ...
    selectMemberPermissions(username: string) {
        return this.projectService
            ...
            .pipe(
                switchMap((projectId) =>
                    this.projectsService.select(['projectsById', projectId, 'permissions', username]),
                ),
            );
    }
    ...
    get members() {
        return this.projectService.selectCurrentProject().pipe(map((project) => Object.keys(project.permissions)));
    }
    ...
    private async handleUpdateMemberPermissions(username: string, permission: PermissionLevel) {
        const projectId = await this.projectService.get('projectId');
