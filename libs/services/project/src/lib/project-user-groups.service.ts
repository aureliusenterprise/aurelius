...
export class ProjectUserGroupsService extends BasicStore<ProjectUserGroupsStoreContext> {
    ...
    private init() {
        ...
        this.projectService
            .select('projectId')
            .pipe(
                switchMap((projectId) => this.watch(['userGroupsPerProject', projectId, 'userGroupsById'])),
    ...
    async updateUserGroup(userGroup: UserGroup) {
        ...
        if (userGroupId) {
            this.update({
                ...
                path: ['userGroupsPerProject', projectId, 'userGroupsById', userGroupId],
                payload: userGroup,
            });
        } else {
    ...
    get userGroups(): Observable<UserGroup[]> {
        return this.projectService.select('projectId').pipe(
            switchMap((projectId) => this.select(['userGroupsPerProject', projectId, 'userGroupsById'])),
            map(Object.values),
        );
    }
    ...
    private async handleDeleteUserGroup(userGroup: UserGroup) {
        ...
        const deleteUserGroupResponse = await deleteUserGroup(projectId, userGroupId).toPromise();
