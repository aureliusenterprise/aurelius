...
hidden_elements_config = MetricConfig(
    description=(
        "An element is non-compliant if it is present in the model but not present in any ArchiMate Views."
    ),
    id_column="id",
    data={
...
def find_elements_in_view(view: List[Dict[str, Any]]) -> Generator[str, None, None]:
    """
    ...
    :param list L: archimate View, represented as list of dictionaries, where each dict is a node
        with potential nested lists with more nodes
        ...
        if d["@xsi_type"] == "ar3_Element":
            ...
            for k, v in d.items():
                if isinstance(v, list) and k == "ar3_node":
                    ...
                    yield v  # type: ignore[reportGeneralTypeIssues]
                # END IF
            # END LOOP
        # END IF
    # END LOOP
...
class ElementsNotInAnyViewMetric(Metric):
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        elements_in_views = [
            list(find_elements_in_view(view)) for view in model.views.nodes if view is not None
        ]
        ...
        not_in_views = [elem_id for elem_id in elems.id if elem_id not in elements_in_views]

        hidden_elements = elems[elems.id.isin(not_in_views)][["id", "name", "type_"]]
