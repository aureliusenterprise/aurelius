...
cycles_config = MetricConfig(
    description=(
        "The set of elements connected via aggregation or composition relationships that form a cycle."
    ),
    id_column=None,
    data={
        "elements_id": MetricColumnConfig(
            displayName="Elements ID", description="The identifiers of the elements in the cycle."
        ),
        "cycle_size": MetricColumnConfig(
            displayName="Cycle Size", description="The number of elements in the cycle."
        ),
        "element_types": MetricColumnConfig(
            displayName="Element Types", description="The ArchiMate types of the elements in the cycle."
        ),
...
class CycleDetectionMetric(Metric):
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        model2_sliced = ArchimateUtils.sliceByEdgeType(model, [RelationshipType.SPECIALIZATION])
        ...
        cycles_list: List[List[str]] = list(nx.simple_cycles(G_sliced))

        G2_sliced = GraphUtils.toNXGraph(model2_sliced)
        cycles2_list: List[List[str]] = list(nx.simple_cycles(G2_sliced))
        ...
        for nodes in cycles_list:
            ...
            filtered_edges_temp = [x for x in edges_temp if (x[0] in nodes and x[1] in nodes)]
            rel_types.append(", ".join({i[2]["type_name"] for i in filtered_edges_temp}))

            # get the unique element types present in the cycle
            element_types.append(", ".join({G_sliced.nodes[i]["type_name"] for i in nodes}))
        # END LOOP

        # expand cycle list with attributes of elements & relationships
        rel_types2: List[str] = []
        element_types2: List[str] = []
        for nodes in cycles2_list:
            # get the relationship types present between elements in the cycle
            edges_temp = list(G2_sliced.edges(nodes, data=True))
            filtered_edges_temp = [x for x in edges_temp if (x[0] in nodes and x[1] in nodes)]
            rel_types2.append(", ".join({i[2]["type_name"] for i in filtered_edges_temp}))

            # get the unique element types present in the cycle
            element_types2.append(", ".join({G2_sliced.nodes[i]["type_name"] for i in nodes}))
        # END LOOP

        cycles = pd.DataFrame(
            {
                "elements_id": [", ".join(i) for i in cycles_list],
                "cycle_size": [len(i) for i in cycles_list],
                "element_types": element_types,
                "rel_types": rel_types,
            }
        )
        cycles2 = pd.DataFrame(
            {
                "elements_id": [", ".join(i) for i in cycles2_list],
