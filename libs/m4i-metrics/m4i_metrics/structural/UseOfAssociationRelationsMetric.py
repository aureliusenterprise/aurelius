...
invalid_associations_agg_config = MetricConfig(
    description=(
        "These relationships are association relationships between business layer, application layer, "
        "technology layer, physical layer nodes, or junctions."
    ),
    id_column="id_rel",
    data={
...
class UseOfAssociationRelationsMetric(Metric):
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        business_layer = [
            ElementType.BUSINESS_ACTOR["typename"],
            ElementType.BUSINESS_ROLE["typename"],
            ElementType.BUSINESS_COLLABORATION["typename"],
            ElementType.BUSINESS_INTERFACE["typename"],
            ElementType.BUSINESS_PROCESS["typename"],
            ElementType.BUSINESS_FUNCTION["typename"],
            ElementType.BUSINESS_INTERACTION["typename"],
            ElementType.BUSINESS_EVENT["typename"],
            ElementType.BUSINESS_SERVICE["typename"],
            ElementType.BUSINESS_OBJECT["typename"],
            ElementType.CONTRACT["typename"],
            ElementType.REPRESENTATION["typename"],
            ElementType.PRODUCT["typename"],
        ]

        application_layer = [
            ElementType.APPLICATION_COMPONENT["typename"],
            ElementType.APPLICATION_COLLABORATION["typename"],
            ElementType.APPLICATION_INTERFACE["typename"],
            ElementType.APPLICATION_FUNCTION["typename"],
            ElementType.APPLICATION_INTERACTION["typename"],
            ElementType.APPLICATION_PROCESS["typename"],
            ElementType.APPLICATION_EVENT["typename"],
            ElementType.APPLICATION_SERVICE["typename"],
            ElementType.DATA_OBJECT["typename"],
        ]

        technology_layer = [
            ElementType.NODE["typename"],
            ElementType.DEVICE["typename"],
            ElementType.SYSTEM_SOFTWARE["typename"],
            ElementType.TECHNOLOGY_COLLABORATION["typename"],
            ElementType.TECHNOLOGY_INTERFACE["typename"],
            ElementType.PATH["typename"],
            ElementType.COMMUNICATION_NETWORK["typename"],
            ElementType.TECHNOLOGY_FUNCTION["typename"],
            ElementType.TECHNOLOGY_PROCESS["typename"],
            ElementType.TECHNOLOGY_INTERACTION["typename"],
            ElementType.TECHNOLOGY_EVENT["typename"],
            ElementType.TECHNOLOGY_SERVICE["typename"],
            ElementType.ARTIFACT["typename"],
        ]

        physical_layer = [
            ElementType.EQUIPMENT["typename"],
            ElementType.FACILITY["typename"],
            ElementType.DISTRIBUTION_NETWORK["typename"],
            ElementType.MATERIAL["typename"],
        ]

        junctions = [ElementType.OR_JUNCTION["typename"], ElementType.AND_JUNCTION["typename"]]

        all_layers = business_layer + application_layer + technology_layer + physical_layer + junctions

        type_agg["SRCinLayers"] = type_agg["type_src"].apply(lambda x: x in all_layers)
        type_agg["TGTinLayers"] = type_agg["type_tgt"].apply(lambda x: x in all_layers)

        associations_agg = type_agg[type_agg["type_rel"] == RelationshipType.ASSOCIATION["typename"]]

        allRelsCount = len(associations_agg)

        invalid_associations_agg = associations_agg[
            (associations_agg["SRCinLayers"]) & (associations_agg["TGTinLayers"])
        ]
