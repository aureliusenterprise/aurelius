...
invalid_junctions_direction_agg_config = MetricConfig(
    ...
    data={
        "id_junction": MetricColumnConfig(
            displayName="Junction ID", description="The identifier of the junction"
        ),
        "type_junction": MetricColumnConfig(
            displayName="Junction type", description="The type of the junction"
        ),
...
class MissconnectedJunctionsMetric(Metric):
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        elems["incomingRelsCount"] = elems["id"].apply(lambda x: len(rels[rels["target"] == x]))
        elems["outgoingRelsCount"] = elems["id"].apply(lambda x: len(rels[rels["source"] == x]))

        junctions_agg = elems[
            (elems["type_"] == ElementType.OR_JUNCTION["typename"])
            | (elems["type_"] == ElementType.AND_JUNCTION["typename"])
        ]

        junctions_agg.rename(
            columns={"id": "id_junction", "name": "name_junction", "type_": "type_junction"}, inplace=True
        )
