...
no_relations_config = MetricConfig(
    ...
    data={
        "concept_id": MetricColumnConfig(displayName="ID", description="The identifier of the element"),
        "concept_name": MetricColumnConfig(displayName="Element name", description="The name of the element"),
        "concept_type": MetricColumnConfig(
            displayName="Element type", description="The ArchiMate type of the element"
        ),
        "view_name": MetricColumnConfig(displayName="View name", description="The name of the View"),
        "view_path": MetricColumnConfig(
            displayName="View Path", description="The organizational path of the View"
        ),
    },
)
...
invalid_relations_config = MetricConfig(
    description=(
        "These concepts are nested in a View but have a relationship that is not allowed. "
        "Allowed relationships: Aggregation, Composition, Assignment, and Realization."
    ),
    id_column="concept_id",
    data={
        "concept_id": MetricColumnConfig(displayName="ID", description="The identifier of the element"),
        ...
        "relation_name": MetricColumnConfig(
            displayName="Relationship name", description="The name of the relationship"
        ),
        "relation_type": MetricColumnConfig(
            displayName="Relationship type", description="The ArchiMate type of the relationship"
        ),
    },
)


def get_child_parent_pair(
    nodes: List[Dict[str, Any]], parent: Optional[str] = None
...
def get_view_paths(model: Any) -> pd.DataFrame:
    # get view names
    # using model.organizations.idRef == model.views.id
    # organizations could be arbitrarily long levels
    view_levels = model.views[["id", "name"]].merge(model.organizations, left_on="id", right_on="idRef")
    #  retun  path by concatenating columns with level in their name
    view_levels["path"] = view_levels.loc[:, view_levels.columns.str.contains("level")].apply(
        lambda x: x.str.cat(sep="/"), 1
    )
    view_levels = view_levels.rename(columns={"id": "view_id", "name": "view_name", "path": "view_path"})
    return view_levels[["view_id", "view_path", "view_name"]]


def find_nested_elements(model: Any) -> pd.DataFrame:
    # filter nodes in views that dont nest
    # nested_elems_nodes = [] #
    def filter(view):
        return [dict_elem for dict_elem in view if "ar3_node" in dict_elem]

    # get all child parent relations for nested elements
    view_nested = {
        index: get_child_parent_pair(filter(view))
        for index, view in model.views.nodes.iteritems()
        if view is not None
    }

    def remove_top_level_nodes(list_of_tuple):
        return [(x, y) for x, y in list_of_tuple if y is not None]

    view_nested = {key: remove_top_level_nodes(value) for key, value in view_nested.items()}
...
def get_elems_rels(model: Any) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
class NestedElementsInViewMetric(Metric):
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        if df.empty or elems.empty or rels.empty:
            ...
            df = df.rename(columns={"id": "relation_id", "name": "relation_name", "type": "relation_type"})
            ...
            df = df.merge(elems, how="left", left_on=["target"], right_on=["id"])
            df = df.rename(columns={"id": "concept_id", "name": "concept_name", "type": "concept_type"})

            allowed_relations = [
                RelationshipType.COMPOSITION["typename"],
            ...
            df_invalid_relations = df[~df["relation_type"].isin(allowed_relations)]
