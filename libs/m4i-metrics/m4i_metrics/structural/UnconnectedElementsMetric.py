...
clusters_config = MetricConfig(
    ...
    id_column="id",
    data={
        "group": MetricColumnConfig(displayName="Group", description="The id of the cluster"),
        "id": MetricColumnConfig(displayName="Concept id", description="The id of the concept"),
        "name": MetricColumnConfig(displayName="Element name", description="The name of the element"),
        "type": MetricColumnConfig(
            displayName="Element type", description="The ArchiMate type of the element"
        ),
...
def cluster_nodes(clusters: Any) -> List[Tuple[int, str]]:  # type: ignore[reportGeneralTypeIssues]
    """
    ...
    for index, cluster in enumerate(clusters, 1):
        ...
        for node in cluster:
            yield (cluster_id, node)  # type: ignore[reportGeneralTypeIssues]
        # END LOOP
    # END LOOP
...
def get_partitions(model: ArchimateModel) -> pd.DataFrame:
    """
...
def edges_to_nodes(model: ArchimateModel) -> ArchimateModel:
    ...
    for index, edge in model.edges.iterrows():
        ...
        incoming = {edge_id_key: incoming_edge_id, source_key: source, target_key: edge_id}

        outgoing_edge_id = f"{edge_id}_out"
        outgoing = {edge_id_key: outgoing_edge_id, source_key: edge_id, target_key: target}

        # Append the new node and edges to the result, indexed by ID
        nodes = {**nodes, edge_id: node}  # type: ignore[reportGeneralTypeIssues]
        edges = {**edges, incoming_edge_id: incoming, outgoing_edge_id: outgoing}
    # END LOOP

    # Also include the already existing nodes
    for index, node in model.nodes.iterrows():
        node_id = node[node_id_key]
        nodes = {**nodes, node_id: {node_id_key: node_id}}  # type: ignore[reportGeneralTypeIssues]
    # END LOOP

    # Return the model including the new nodes and edges. Keep other settings.
    model_with_edges_as_nodes = copy.copy(model)

    model_with_edges_as_nodes.nodes = pd.DataFrame(nodes.values(), columns=model.nodes.columns)
    model_with_edges_as_nodes.edges = pd.DataFrame(edges.values(), columns=model.edges.columns)
...
class UnconnectedElementsMetric(Metric):
    ...
    unconnected_elems: pd.DataFrame | None = None  # type: ignore[reportGeneralTypeIssues]
    ...
    def calculate(model: ArchimateModel) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        partition_count = partitions["group"].unique().size  # type: ignore[reportOptionalSubscript, reportOptionalMemberAccess]
        has_multiple_partitons = partition_count > 1

        if has_multiple_partitons:
            clusters = pd.merge(left=model.nodes, right=partitions, how="left", on="id")

            # Return the type name rather than the complex type object
            clusters["type"] = clusters["type"].apply(lambda x: x["typename"])  # type: ignore[reportOptionalSubscript, reportOptionalMemberAccess]
