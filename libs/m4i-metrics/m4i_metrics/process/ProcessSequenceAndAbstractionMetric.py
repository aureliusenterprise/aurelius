...
invalid_processes_agg_config: MetricConfig = MetricConfig(
    description=(
        "These relationships are not triggering/aggregation/composition or specialization "
        "between similar-type processes. There are no junctions between the processes"
    ),
    id_column="id_rel",
    data={
...
invalid_processes_junctions_agg_config: MetricConfig = MetricConfig(
    description=(
        "These relationships are not triggering/aggregation/composition or specialization "
        "between similar-type processes. There are junctions between the processes"
    ),
    id_column="id_rel",
    data={
        "id_rel": MetricColumnConfig(
...
def generateInvalidDF_(
    ...
    processes_agg: pd.DataFrame = type_agg[  # type: ignore[reportAssignmentType]
        ((type_agg["type_src"] == processtype) & (type_agg["type_tgt"] == processtype))
    ]

    invalid_processes_agg: pd.DataFrame = processes_agg[  # type: ignore[reportAssignmentType]
        ~(
    ...
    # but different process_starts and process_ends
...
class ProcessSequenceAndAbstractionMetric(Metric):
    id: str = "71239737-0106-4d0e-838e-7d67f20f42fc"
    label: str = "Process Sequence & Abstraction"
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        invalid_processes_agg = pd.concat(
            [invalid_business_agg, invalid_application_agg, invalid_technology_agg], sort=False
        )

        invalid_processes_junctions_agg = pd.concat(
            [
                invalid_business_junction_agg,
