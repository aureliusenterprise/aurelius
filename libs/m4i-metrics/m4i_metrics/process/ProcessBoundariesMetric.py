...
parent_non_compliant_elem_config = MetricConfig(
    description=(
        "These process steps trigger other process steps at different abstraction levels in the model"
    ),
    ...
    data={
        ...
        "name_source": MetricColumnConfig(
            displayName="Source name", description="The name of the source element"
        ),
        "type__source": MetricColumnConfig(
            displayName="Source type", description="The ArchiMate type of the source element"
        ),
        "name_target": MetricColumnConfig(
            displayName="Target name", description="The name of the target element"
        ),
        "type__target": MetricColumnConfig(
            displayName="Target type", description="The ArchiMate type of the target element"
        ),
    },
)
...
class ProcessBoundariesMetric(Metric):
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        trigger_rels = rels[
            np.logical_and(
                rels.type_ == RelationshipType.TRIGGERING["typename"],
                np.logical_and(rels.source.isin(behavior_ids), rels.target.isin(behavior_ids)),
            )
        ][["id", "source", "target"]]
        ...
        # B <- D
        parent_target = trigger_rels.merge(comp_agg_rels, how="left", on="target", suffixes=["_B", "_D"])
        ...
        # trigger relation source and target must belong to the same sup process,
        # thus, there must be a composition or aggregation to the same concept
        parent_source_target = parent_source.merge(parent_target, how="inner", on="id")
        parent_source_target[config.METRIC_TAG] = (
            parent_source_target.source_C == parent_source_target.source_D
        )

        parent_non_compliant = parent_source_target[
            np.logical_and(
                np.logical_not(parent_source_target[config.METRIC_TAG]),
                np.logical_or(
                    parent_source_target.source_C.notnull(), parent_source_target.source_D.notnull()
                ),
            )
        ]
