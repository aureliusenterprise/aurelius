...
invalid_actors_roles_processes_functions_agg_config = MetricConfig(
    description=(
        "These relationships are not assignment between business actors and business roles or "
        "business actors/business roles and business processes/business functions"
    ),
    id_column="id_rel",
    data={
...
class ActorAndRoleAssignmentMetric(Metric):
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        business_agg = business_elems.merge(rels, how="inner", left_on="id", right_on="source")
        business_agg.rename(
            columns={
                "id_x": "id_src",
                "name": "name_src",
        ...
        business_agg = business_agg.merge(business_elems, how="inner", left_on="target", right_on="id")
        business_agg.rename(columns={"id": "id_tgt", "name": "name_tgt", "type_": "type_tgt"}, inplace=True)
        business_agg = business_agg[
        ...
        actors_roles_processes_functions_agg = pd.concat(
            [
                actors_roles_agg,
                actors_processes_agg,
                roles_processes_agg,
                actors_functions_agg,
                roles_functions_agg,
            ]
        )
        ...
        return {
            "Relationships between business actors, business roles, "
