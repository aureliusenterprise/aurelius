...
invalid_distribution_networks_agg_config: MetricConfig = MetricConfig(
    description=(
        "These relationships are not aggregation/composition/flow/triggering or specialization "
        "between distribution networks. There are no junctions between the distribution networks"
    ),
    id_column="id_rel",
    data={
...
invalid_distribution_networks_junctions_agg_config: MetricConfig = MetricConfig(
    description=(
        "These relationships are not aggregation/composition/flow/triggering or specialization "
        "between distribution networks. There are junctions between the distribution networks"
    ),
    id_column="id_rel",
    data={
        "id_rel": MetricColumnConfig(
        ...
        "name_start": MetricColumnConfig(
            ...
            description=(
                "The name of the distribution network start of the path of which the relationship is part of"
            ),
        ),
        "type_start": MetricColumnConfig(
            displayName="Start type",
            description=(
                "The type of the distribution network start of the path of which the relationship is part of"
            ),
        ),
        "type_src": MetricColumnConfig(
            displayName="Source type", description="The type of the source element of the relationship"
        ...
        "name_end": MetricColumnConfig(
            ...
            description=(
                "The name of the distribution network end of the path of which the relationship is part of"
            ),
        ),
        "type_end": MetricColumnConfig(
            displayName="End type",
            description=(
                "The type of the distribution network end of the path of which the relationship is part of"
            ),
        ),
    },
)
...
def generateinvalidDF_(elems: pd.DataFrame, type_agg: pd.DataFrame) -> Tuple[int, pd.DataFrame, pd.DataFrame]:
    distribution_networks_agg: pd.DataFrame = type_agg[  # type: ignore[reportAssignmentType]
        (
    ...
    invalid_distribution_networks_agg: pd.DataFrame = distribution_networks_agg[  # type: ignore[reportAssignmentType]
        ~(
    ...
    distribution_network_agg: pd.DataFrame = elems[  # type: ignore[reportAssignmentType]
        elems["type_"] == ElementType.DISTRIBUTION_NETWORK["typename"]
    ]
    for _, row in distribution_network_agg.iterrows():
    ...
    # but different distribution_network_starts and distribution_network_ends

    invalid_junctions_agg: pd.DataFrame = junctions_agg[  # type: ignore[reportAssignmentType]
        ~(
    ...
    return (
        (len(distribution_networks_agg) + len(junctions_agg)),
        invalid_distribution_networks_agg,
        invalid_junctions_agg,
    )
...
def breadth_first_search_(startNodeID: str, elems: pd.DataFrame, type_agg: pd.DataFrame) -> pd.DataFrame:
    visited: List[str] = []
    currentLevel: int = 1
...
def findNeighbours_(NodesrcID: str, type_agg: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    ...
    for _, row in adjacent_target_agg.iterrows():
...
class DistributionNetworksMetric(Metric):
    id: str = "926f292e-061d-47e7-a1a4-2db23e76879b"
    label: str = "Distribution Networks"
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        type_agg = elems.merge(rels, how="inner", left_on="id", right_on="source")
        type_agg.rename(
            columns={
                "id_x": "id_src",
                "name": "name_src",
        ...
        (allRelsCount, invalid_distribution_networks_agg, invalid_distribution_networks_junctions_agg) = (
            generateinvalidDF_(elems, type_agg)
        )
