...
invalid_facilities_processes_agg_config = MetricConfig(
    description=(
        "These relationships are triggering between facilities and processes. "
        "There are no junctions between the facilities and processes"
    ),
    id_column="id_rel",
    data={
...
invalid_facilities_junctions_agg_config = MetricConfig(
    description=(
        "These relationships are triggering between facilities and processes. "
        "There are junctions between the facilities and processes"
    ),
    id_column="id_rel",
    data={
        "id_rel": MetricColumnConfig(
        ...
        "name_start": MetricColumnConfig(
            ...
            description=(
                "The name of the facility start/process start of the path "
                "of which the relationship is part of"
            ),
        ),
        "type_start": MetricColumnConfig(
            displayName="Start type",
            description=(
                "The type of the facility start/process start of the path "
                "of which the relationship is part of"
            ),
        ),
        "type_src": MetricColumnConfig(
        ...
        "name_end": MetricColumnConfig(
            ...
            description=(
                "The name of the facility end/process end of the path of which the relationship is part of"
            ),
        ),
        "type_end": MetricColumnConfig(
            displayName="End type",
            description=(
                "The type of the facility end/process end of the path of which the relationship is part of"
            ),
        ),
    },
)
...
def generateinvalidDF_(
    processtype: str, elems: pd.DataFrame, type_agg: pd.DataFrame
) -> Tuple[int, pd.DataFrame, pd.DataFrame]:
    facilities_agg: pd.DataFrame = type_agg[  # type: ignore[reportAssignmentType]
        ((type_agg["type_src"] == ElementType.FACILITY["typename"]) & (type_agg["type_tgt"] == processtype))
        | ((type_agg["type_src"] == processtype) & (type_agg["type_tgt"] == ElementType.FACILITY["typename"]))
    ]

    invalid_facilities_agg = facilities_agg[
        facilities_agg["type_rel"] == RelationshipType.TRIGGERING["typename"]
    ]
    ...
    facility_agg: pd.DataFrame = elems[elems["type_"] == ElementType.FACILITY["typename"]]  # type: ignore[reportAssignmentType]

    for _, row in facility_agg.iterrows():
    ...
    # but different facility_starts/process_starts and facility_ends/event_ends

    invalid_junctions_agg = junctions_agg[
        junctions_agg["type_rel"] == RelationshipType.TRIGGERING["typename"]
    ]

    return (  # type: ignore[reportAssignmentType]
        (len(facilities_agg) + len(junctions_agg)),
        invalid_facilities_agg,
...
class MaterialFlowMetric(Metric):
    id: str = "29cab7c5-7d35-4f03-8644-2e63baac8056"
    label: str = "Material Flow"
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        invalid_facilities_processes_agg = pd.concat(
            [invalid_business_agg, invalid_application_agg, invalid_technology_agg]
        )

        invalid_facilities_junctions_agg = pd.concat(
