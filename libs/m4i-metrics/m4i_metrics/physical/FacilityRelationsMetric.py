...
invalid_between_facilities_agg_config = MetricConfig(
    description=(
        "These relationships are not aggregation/composition/realization or specialization between facilities"
    ),
    id_column="id_rel",
    data={
...
class FacilityRelationsMetric(Metric):
    ...
    def calculate(model: Any) -> Dict[str, Any]:  # type: ignore[reportIncompatibleMethodOverride]
        ...
        facility_agg = facility_elems.merge(rels, how="inner", left_on="id", right_on="source")
        facility_agg.rename(
            columns={
                "id_x": "id_src",
                "name": "name_src",
        ...
        facility_agg = facility_agg.merge(facility_elems, how="inner", left_on="target", right_on="id")
        facility_agg.rename(columns={"id": "id_tgt", "name": "name_tgt", "type_": "type_tgt"}, inplace=True)
