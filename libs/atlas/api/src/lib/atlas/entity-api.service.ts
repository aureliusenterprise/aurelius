...
import { AtlasEntityWithEXTInformation, EntityMutationResponse, GetOptions } from '../types';
...
export class EntityAPIService {
    ...
    getEntityById(guid: string, { forceUpdate = false }: GetOptions = {}): Observable<AtlasEntityWithEXTInformation> {
        const path = `${this.BASE_PATH}/guid/${guid}`;

        return this.http.cache(forceUpdate).get<AtlasEntityWithEXTInformation>(path);
    }

    saveEntity(entity: AtlasEntityWithEXTInformation): Observable<EntityMutationResponse> {
