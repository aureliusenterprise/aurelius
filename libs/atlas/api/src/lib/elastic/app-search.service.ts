...
export type AppSearchFilterValue<T extends AppSearchDocument, K extends keyof T> =
    T[K] extends Array<infer V> ? V : T[K];

export type AppSearchFilterValues<T extends AppSearchDocument, K extends keyof T> = AppSearchFilterValue<T, K>[];
...
export type AppSearchFilters<T extends AppSearchDocument> = {
    [A in 'all' | 'any' | 'none']?: (AppSearchFilters<T> | AppSearchFilterSelection<T>)[];
};
...
export interface AppSearchQuery<T extends AppSearchDocument, P extends Partial<T> = T> {
...
export interface AppSearchResultsFacetData<T extends AppSearchDocument, V extends T[keyof T]> {
    readonly value: V;
    readonly count: number;
}

export interface AppSearchResultsFacet<T extends AppSearchDocument, V extends T[keyof T]> {
...
export interface AppSearchResults<T extends AppSearchDocument, P extends Partial<T> = T> {
    readonly meta?: Meta;
...
@Injectable()
export class AppSearchService<T extends AppSearchDocument, P extends Partial<T> = T> {
    constructor(
        private readonly http: AppSearchClient,
        @Inject($APP_SEARCH_PATH) private readonly PATH: string,
    ) {}

    search(queryObject: AppSearchQuery<T, P>): Observable<AppSearchResults<T, P>> {
