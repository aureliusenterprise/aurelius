...
import { AbstractSelectComponent, defaultSelectContext, SelectContext } from '../select';
...
export class BranchSelectComponent extends AbstractSelectComponent<Branch> implements OnInit {
    ...
    ngOnInit() {
        ...
        this.createBranchModal.submission.pipe(untilDestroyed(this)).subscribe((branch) => {
            this.control.patchValue(branch);
        });

        // Whenever the list of branches updates, update the options in the select component
        this.branchesService.branches.pipe(untilDestroyed(this)).subscribe((branches) => (this.select.data = branches));
