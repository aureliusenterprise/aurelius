...
export class UserSearchInputComponent implements OnInit, OnDestroy {
    @Output() suggestions: EventEmitter<UserSearch[]> = new EventEmitter<UserSearch[]>();

    constructor(private readonly searchService: UserSearchInputService) {}

    ngOnInit() {
        this.searchService.select('suggestions').pipe(untilDestroyed(this)).subscribe(this.suggestions);
    }
