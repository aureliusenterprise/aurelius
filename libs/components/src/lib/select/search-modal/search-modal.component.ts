...
export class SearchModalComponent<T extends Dictionary<any>> extends AbstractModal {
    ...
    @ViewChild(DatePickerComponent, { static: false }) set datepicker(datepicker: DatePickerComponent) {
        if (datepicker) {
            // Whenever the user selects a date, search for items within a timespan of a day
            datepicker.dateChanged.pipe(untilDestroyed(datepicker)).subscribe((lowerBound) => {
    ...
    @ViewChild(FuzzySearchInputComponent, { static: false }) set searchInput(
        ...
        if (searchInput) {
            ...
            searchInput.isUpdatingSearchIndex$.pipe(untilDestroyed(searchInput)).subscribe(this.isLoading$);

            searchInput.queryChanged.pipe(untilDestroyed(searchInput)).subscribe((query) => (this.searchQuery = query));
    ...
    set searchTable(searchTable: SortableTableComponent<T>) {
        if (searchTable) {
            searchTable.rowClicked.pipe(untilDestroyed(searchTable)).subscribe((value) => {
    ...
    private updateFilteredRows() {
        if (!this.searchResult || this.searchResult.length === 0) {
            ...
            this.filteredRows = this.searchResult.map((suggestion) => this.rowsByKey[suggestion]);
        }
    }
