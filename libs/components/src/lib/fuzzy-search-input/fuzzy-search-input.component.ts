...
import { AbstractControlShell, ControlShellContext, defaultControlShellContext } from '../control-shell';
import { FuzzySearchInputService, FuzzySearchTokenizerConfig } from './fuzzy-search-input.service';
...
export class FuzzySearchInputComponent<T = string> extends AbstractControlShell implements OnInit, OnDestroy {
    /**
    ...
    ngOnInit() {
        ...
        this.searchService
            ...
            .subscribe((query) => this.control.patchValue(query, { emitEvent: false }));
        ...
        this.searchService.select('query').pipe(untilDestroyed(this)).subscribe(this.queryChanged);

        this.searchService.select('suggestions').pipe(untilDestroyed(this)).subscribe(this.suggestionsChanged);

        this.isUpdatingSearchIndex$ = this.searchService.select('isCreatingSearchIndex');

        this.isUpdatingSuggestions$ = this.searchService.select('isUpdatingSuggestions');
    }
