...
import { iterate, substrings, SubType, untilDestroyed } from '@models4insight/utils';
...
export const stringTokenizer: FuzzySearchTokenizerFunction<string> = (item: string) => {
...
function* resolveSearchQueryTokens(
    ...
    for (const token of queryTokens) {
        ...
        let searchTokens = orderBy(containsSearchIndex.get(token) ?? [], 'length', 'asc');
        ...
        const result = searchTokens.filter((searchToken) => !seenTokens.has(searchToken));
...
// TODO: Add Angular decorator.
@Injectable()
export class FuzzySearchInputService<T> extends BasicStore<FuzzySearchInputStoreContext<T>> implements OnDestroy {
    constructor() {
    ...
    private async handleCreateSearchIndex(items: T[], tokenizerConfig: FuzzySearchTokenizerConfig<T>) {
        ...
        const exactSearchIndex = InvertedIndex.from(items, [exactSearchTokenizer, identity]);
        ...
        const tokenizedSearchIndex = InvertedIndex.from(items, [tokenizedSearchTokenizer, identity]);
    ...
    private async handleCreateContainsSearchIndex(searchTokens: string[]) {
        ...
        const containsSearchIndex = InvertedIndex.from(searchTokens, [containsSearchItemTokenizer, (query) => [query]]);
    ...
    private async handleCreateFuzzySearchIndex(searchTokens: string[]) {
        ...
        const fuzzySearchIndex = SymSpell.from(filteredSearchTokens, symSpellOptions);
    ...
    private async handleUpdateSuggestions(query: string) {
        ...
        if (query) {
            const [exactSearchIndex, tokenizedSearchIndex, containsSearchIndex, fuzzySearchIndex] = await Promise.all([
            ...
            suggestions = exactSearchIndex.get([deburr(query.trim().toLowerCase())]) ?? [];
