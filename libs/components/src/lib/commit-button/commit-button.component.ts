...
export class CommitButtonComponent implements OnInit {
    ...
    ngOnInit() {
        this.isCommittingModel$ = this.commitModelService.select('isCommittingModel').pipe(shareReplay());
    }
    ...
    onCommitButtonClicked() {
        ...
        const options: CommitOptions = {
            ...formValue,
            conflictResolutionTemplate: formValue.conflictResolutionTemplate?.template,
