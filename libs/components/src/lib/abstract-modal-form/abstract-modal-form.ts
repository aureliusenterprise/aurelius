...
@Directive()
export abstract class AbstractModalForm<T> extends AbstractModal implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.modal.confirmed.pipe(untilDestroyed(this)).subscribe(() => this.submit());

        // Whenever the modal is cancelled, clear the form
        this.modal.cancelled.pipe(untilDestroyed(this)).subscribe(() => this.clear());
    }
