...
import { faInfo, faShieldAlt, faUserPlus, faUsersCog } from '@fortawesome/free-solid-svg-icons';
import { Branch, BranchPermissionLevel, PermissionLevel } from '@models4insight/repository';
...
import { defaultDescriptionInputContext, DescriptionInput, DescriptionInputContext } from '../description-input';
...
export class CreateBranchModalComponent extends AbstractModalForm<Branch> implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.submission.pipe(untilDestroyed(this)).subscribe((branch) => this.branchesService.updateBranch(branch));
    }

    ngOnDestroy() {}

    addMember(username: string, permissionLevel: BranchPermissionLevel = PermissionLevel.BUSINESS_USER) {
        this.members.addControl(username, this.formBuilder.control(permissionLevel));
    }
    ...
    protected initForm(): UntypedFormGroup {
        ...
        const branches$ = this.branchesService.branches.pipe(map((branches) => branches.map((branch) => branch.name)));
    ...
    protected onSubjectChanged(branch: Branch) {
        ...
        Object.keys(this.members.controls).forEach((username) => this.removeMember(username));
        if (branch) {
            const members = branch.members || {};
            Object.entries(members).forEach(([username, permissionLevel]) => this.addMember(username, permissionLevel));
        }
    }
}
