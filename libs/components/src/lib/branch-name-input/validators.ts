...
export function unique(
    ...
    return async ({ value: branchName }: AbstractControl) => {
        ...
        return branchNames && branchName !== currentBranchName && branchNames.includes(branchName)
            ? { unique: true }
            : null;
    };
}
...
export function leadingOrTrailingWhitespace({ value: branchName }: AbstractControl) {
    return branchName && branchName.length !== branchName.trim().length ? { leadingOrTrailingWhitespace: true } : null;
}
...
export function multipleWhitespaces({ value: branchName }: AbstractControl) {
    ...
    return !!matches && !!matches.find((match: string) => match.length > 1) ? { multipleWhitespaces: true } : null;
}
