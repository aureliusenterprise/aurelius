...
export function unique(username$: string | Observable<string>, projects$: Project[] | Observable<Project[]>) {
    return async (control: AbstractControl) => {
        ...
        const isNotUnique = projects.some(
            (project) => project.committer.username === username && project.name === control.value,
        );
...
export function leadingOrTrailingWhitespace(control: AbstractControl) {
    ...
    return name && name.length !== name.trim().length ? { leadingOrTrailingWhitespace: true } : null;
}
...
export function multipleWhitespaces(control: AbstractControl) {
    ...
    if (name) {
        ...
        return match && match.some((sample: string) => sample.length > 1) ? { multipleWhitespaces: true } : null;
    }
    return null;
}
