...
export function unique(
    ...
    return ({ value: groupName }: AbstractControl) => {
        ...
        return groupNames && groupName !== currentGroupName && groupNames.some((name) => name === groupName)
            ? { unique: true }
            : null;
    };
}
...
export function leadingOrTrailingWhitespace({ value: groupName }: AbstractControl) {
    return groupName && groupName.length !== groupName.trim().length ? { leadingOrTrailingWhitespace: true } : null;
}
