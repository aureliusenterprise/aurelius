...
import { ProjectMembersService, ProjectService, ProjectUserGroupsService } from '@models4insight/services/project';
    ...
    const isConnected = ({ members = [] }: UserGroup, visited: string[] = []) => {
        return (
            ...
            members
                ...
                .some((member) => isConnected(groupsByName[member], [...visited, ...members]))
        );
    };
    ...
    private init() {
        ...
        combineLatest([
            ...
            this.projectService
                ...
                .pipe(
                    switchMap((projectId) =>
                        this.projectUserGroupsService.select(['userGroupsPerProject', projectId, 'userGroupsById']),
                    ),
                ),
        ])
            .pipe(
                map(([currentGroupName, groups]) => findConnectedGroups(currentGroupName, groups)),
                untilDestroyed(this),
            )
        ...
        combineLatest([
            ...
            this.projectUserGroupsService.userGroups.pipe(map((groups) => groups.map((group) => group.name))),
        ])
            .pipe(
