...
export class GroupMembersComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.nameTypeIndex$ = this.createUserGroupModalService.select('nameTypeIndex');
    }

    removeMember(event: boolean, username: string) {
        if (event) {
            this.members.removeAt(this.members.controls.findIndex((control) => control.value === username));
        }
    }
