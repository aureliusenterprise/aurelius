...
import { FuzzySearchInputComponent, FuzzySearchTokenizerConfig } from '../../fuzzy-search-input';
...
export class AddGroupMemberComponent implements OnInit {
    ...
    constructor(private readonly createUserGroupModalService: CreateUserGroupModalService) {}

    ngOnInit() {
        this.connectedGroups$ = this.createUserGroupModalService.select('connectedGroups');

        this.filterTerms$ = this.createUserGroupModalService
            .select('currentGroupName')
            .pipe(map((groupName) => [groupName]));

        this.nameTypeIndex$ = this.createUserGroupModalService.select('nameTypeIndex').pipe(shareReplay());
