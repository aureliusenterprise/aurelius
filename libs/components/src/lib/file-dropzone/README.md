...
This module provides the `models4insight-file-dropzone` form component and the `FileDropzone` form control,
for use in forms where a file needs to be uploaded by the user. The `FileDropzone` validates branch names
based on the following criteria:

- A file should be selected
- The file name extension of the selected file should match the allowed extensions

The `models4insight-file-dropzone` component provides user feedback in case the given branch name violates any
