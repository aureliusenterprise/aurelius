...
import { Branch, getProvenance, ModelProvenance } from '@models4insight/repository';
...
export class CompareModalService extends BasicStore<CompareModalStoreContext> {
    ...
    private init() {
        this.loadVersions$.pipe(untilDestroyed(this)).subscribe((branch) => this.handleLoadOtherVersions(branch));
    }
