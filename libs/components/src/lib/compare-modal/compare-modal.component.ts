...
export class CompareModalComponent extends AbstractModalForm<ComparisonContext> implements OnInit {
    ...
    createSubmission() {
        const { baseBranch, otherBranch, baseVersion, otherVersion } = this.form.value;
    ...
    initForm() {
        ...
        this.compareModalService.versions.pipe(untilDestroyed(this)).subscribe(() => otherVersion.reset());
