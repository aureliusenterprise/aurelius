...
export interface SortableTableShellStoreContext<T extends Dictionary<any> = Dictionary<any>> {
...
// TODO: Add Angular decorator.
@Injectable()
export class SortableTableShellService<T extends Dictionary<any> = Dictionary<any>>
    ...
    implements OnDestroy
{
    ...
    private init() {
        ...
        this.rowSelected$
            .pipe(withLatestFrom(this.select('selectedRow', { includeFalsy: true })), untilDestroyed(this))
    ...
    private handleSort(data: Dictionary<any>[], { active, direction }: Sort) {
        const sortedData =
            !active || direction === ''
                ? data
                : data.slice().sort((a, b) => this.compare(a[active], b[active], direction === 'asc'));
