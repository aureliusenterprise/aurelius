...
import { AbstractSelectComponent, defaultSelectContext, Select, SelectContext } from '../select';
...
const permissionSelectOptionsByPermissionLevel: Dictionary<PermissionSelectOption> = {
    ...
    [PermissionLevel.MODEL_USER]: {
        name: 'Model user',
        description: 'Can retrieve models, but cannot commit new versions or move branches',
...
const permissionSelectOptions = Object.values(permissionSelectOptionsByPermissionLevel);
...
export class PermissionSelectComponent extends AbstractSelectComponent<PermissionSelectOption> implements OnInit {
    ...
    ngOnInit() {
        if (this.context === defaultControlShellContext || this.context === defaultSelectContext) {
        ...
        const userPermissionLevel$ = this.user$.pipe(
            switchMap((user) => this.projectMembersService.selectMemberPermissions(user)),
            shareReplay(),
        );
        ...
        this.isDisabled$ = combineLatest([
            ...
            map(
                ([user, userPermission, currentUser, isOwner]) =>
                    currentUser === user || (userPermission === PermissionLevel.OWNER && !isOwner),
            ),
            shareReplay(),
        );
            ...
            map(([isOwner, isDisabled]) =>
                permissionSelectOptions.filter((option) => {
                    const minPermissionLevel =
                        isOwner || isDisabled ? PermissionLevel.OWNER : PermissionLevel.MAINTAINER;
        ...
        userPermissionLevel$
            .pipe(
                map((userPermission) => permissionSelectOptionsByPermissionLevel[userPermission]),
                untilDestroyed(this),
            )
        ...
        (select.valueChanges as Observable<PermissionSelectOption>)
            ...
            .subscribe(([option, user]) => this.projectMembersService.setProjectMemberPermissions(user, option.level));
    }
