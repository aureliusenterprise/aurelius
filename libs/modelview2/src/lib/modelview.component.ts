...
import { faArrowsAlt, faCamera, faExpand, faInfo, faSearch, faSpinner } from '@fortawesome/free-solid-svg-icons';
...
@Component({
    ...
    providers: [ColorViewService, ModelExplorerService, ModelviewService, SaveViewAsImageService],
})
export class ModelviewComponent implements OnInit, OnDestroy {
    ...
    async saveAllViewsAsImages(delay: number) {
        ...
        for (const [id, view] of Object.entries(viewsById)) {
            ...
            const { width, height } = this.viewContainer.nativeElement.getBoundingClientRect();

            const png = await this.saveViewAsImageService.convertViewToImage(this.svg.nativeElement, width, height);
        ...
        saveAsFile(batch, `${projectName} ${branch} ${new Date(version).toUTCString()}.zip`);
    ...
    async saveViewAsImage() {
        ...
        await this.saveViewAsImageService.saveViewAsImage(this.svg.nativeElement, width, height);
    ...
    set isPalletteFixed(isPalletteFixed: boolean) {
        this.colorViewService.update({
            description: `Palette set to fixed: ${Boolean(isPalletteFixed).toString().toUpperCase()}`,
