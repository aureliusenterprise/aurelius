...
import { b64toBlob, bytesToBinaryString, saveAsFile, untilDestroyed } from '@models4insight/utils';
...
async function addMetadata(png: string, metadata: PNGMetadata): Promise<string> {
 ...
 */
async function drawHTML2Canvas(svg: SVGElement, width: number, height: number): Promise<string> {
    ...
    const canvas = await html2canvas(container, {
        ...
        onclone: (clonedDocument) => {
            const clonedContainer = clonedDocument.getElementById('screenshot-container');
 ...
 */
async function drawSVG2Canvas(svg: SVGElement, width: number, height: number): Promise<string> {
...
export class SaveViewAsImageService extends BasicStore<SaveViewAsImageStoreContext> {
    ...
    private init() {
        ...
        this.convert$
            .pipe(
                exhaustMap(([svg, width, height]) => this.handleConvert(svg, width, height)),
                untilDestroyed(this),
            )
