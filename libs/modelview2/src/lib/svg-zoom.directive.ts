import { AfterContentInit, ContentChildren, Directive, ElementRef, OnDestroy } from '@angular/core';
...
export class SVGZoomDirective implements AfterContentInit, OnDestroy {
    ...
    setZoom(zoomTransform: ZoomTransform) {
        this.zoomBehaviorInstance.transform(this.zoomContainer as any, zoomTransform);
    ...
    private applyZoom(zoomTransform: ZoomTransform) {
        ...
        for (const zoomTarget of this.zoomTargets) {
            zoomTarget.nativeElement.setAttribute('transform', zoomTransform.toString());
        }
    }
}
