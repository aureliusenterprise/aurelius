...
import { FuzzySearchInputComponent, FuzzySearchTokenizerConfig } from '@models4insight/components';
...
const modelEntityTokenizerConfig: FuzzySearchTokenizerConfig<ModelElement | ModelRelation> = {
    id: { static: true },
    name: {},
    description: {},
    displayName: {},
...
export class ModelBrowserModelComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.relationships$ = this.modelBrowserModelService.select('suggestedRelations');

        this.selectedEntity$ = this.modelExplorerService.select('selectedEntity', { includeFalsy: true }).pipe(
    ...
    set searchInput(searchInput: FuzzySearchInputComponent<ModelEntity>) {
        if (searchInput) {
            ...
            searchInput.suggestionsChanged.pipe(untilDestroyed(searchInput)).subscribe((suggestions) =>
