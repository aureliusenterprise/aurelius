...
export class ModelBrowserModelService extends BasicStore<ModelBrowserModelStoreContext> {
    ...
    get searchItems() {
        ...
        ]).pipe(map(([elements, relations]) => [...Object.values(elements), ...Object.values(relations)]));
    }

    private async buildSearchResults(suggestions: ModelEntity[]) {
        const buildSearchResultsCategory = async <T extends 'elements' | 'relations'>(category: T) => {
