...
export const MODEL_BROWSER_SELECTION_ADDONS = new InjectionToken<typeof Component>('MODEL_BROWSER_SELECTION_ADDONS');

const propertiesTableConfig: SortableTableShellConfig<KeyValue<string, string>> = {
...
export class ModelBrowserSelectionComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.properties$ = combineLatest([this.selectedEntity$, selectedEntityData$]).pipe(
            map(([entity, data = {}]) => {
                const properties = entity && 'properties' in entity ? entity.properties : {};
