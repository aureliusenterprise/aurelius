...
import { ModelRelation, ModelViewConnection, ModelViewConnectionBendpoint, ModelViewNode } from '../../parsers';
...
function areNested(source: ModelViewNode, target: ModelViewNode) {
    function isNested(
        ...
        return x1 >= x2 && x1 + width1 <= x2 + width2 && y1 >= y2 && y1 + height1 <= y2 + height2;
    }
...
function getShortestLineEndpoints(
    ...
    else if (sourcePos >= targetPos && sourcePos + sourceSurfaceLength >= targetPos + targetSurfaceLength) {
...
function pointToPoint(
    { x: sourceX, y: sourceY, height: sourceHeight = 0, width: sourceWidth = 0 }: PointOnPath,
    { x: targetX, y: targetY, height: targetHeight = 0, width: targetWidth = 0 }: PointOnPath,
): PointToPoint {
    const [x1, x2] = getShortestLineEndpoints(sourceX, sourceWidth, targetX, targetWidth);
    const [y1, y2] = getShortestLineEndpoints(sourceY, sourceHeight, targetY, targetHeight);
...
export class ModelviewConnectionService extends BasicStore<ModelviewConnectionStoreContext> {
        ...
        this.select('viewConnection')
            .pipe(
                switchMap((connection) =>
                    combineLatest([
                        this.modelExplorerService.select(['entitiesById', connection.source]),
                        this.modelExplorerService.select(['entitiesById', connection.target]),
                    ]).pipe(
    ...
    get color() {
        return combineLatest([
            ...
            this.select(['viewConnection', 'id']).pipe(switchMap((id) => this.colorViewService.getColorById(id))),
    ...
    private async handleCalculatePath(
        ...
        if (steps && steps.length) {
            const path = `m0,0${steps.map((step) => `l${step.x2 - step.x1},${step.y2 - step.y1}`).join('')}`;

            const transform = `translate(${steps[0].x1 - minX},${steps[0].y1 - minY})`;
    ...
    private async buildPath(
        ...
        if (!source) {
            source = (await this.modelExplorerService.get(['entitiesById', connection.source])) as
                | ModelViewNode
                | ModelViewConnection;
        }

        if (!target) {
            target = (await this.modelExplorerService.get(['entitiesById', connection.target])) as
                | ModelViewNode
                | ModelViewConnection;
        }

        if (isConnection(source) || isConnection(target) || !areNested(source, target)) {
    ...
    private async buildPathFromSteps(
        steps: Iterable<ModelViewNode | ModelViewConnection | ModelViewConnectionBendpoint>,
    ) {
