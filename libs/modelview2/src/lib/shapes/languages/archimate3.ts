...
const wavedRectangle: BasicShapeFunction = (width, height) =>
    `m0,0v${height - 6}q${width / 4},12,${width / 2},0t${width / 2},0v-${height - 6}z`;
...
const access: ConnectionStyleFunction = (_, relation) => {
    return {
        'marker-start': ['Read', 'ReadWrite'].includes(relation.options.accessType) ? 'access-read' : null,
        'marker-end': ['Write', 'ReadWrite'].includes(relation.options.accessType) ? 'generic-arrow-open' : null,
