...
export class ModelviewNodeService extends BasicStore<ModelviewNodeStoreContext> {
    ...
    get color() {
        return combineLatest([
            ...
            this.select(['viewNode', 'id']).pipe(switchMap((id) => this.colorViewService.getColorById(id))),
            this.select(['modelNode', 'id'], { includeFalsy: true }).pipe(
                switchMap((id) => this.colorViewService.getColorById(id)),
            ),
        ]).pipe(map(([fillColor, viewNodeColor, modelNodeColor]) => viewNodeColor ?? modelNodeColor ?? fillColor));
    }
    ...
    get displayName() {
        ...
        ]).pipe(map(([conceptName, viewElementName]) => conceptName ?? viewElementName ?? ''));
    }
