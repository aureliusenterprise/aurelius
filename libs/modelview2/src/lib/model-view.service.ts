...
export class ModelviewService extends BasicStore<ModelviewStoreContext> {
    ...
    private init() {
        ...
        combineLatest([this.select('viewId', { includeFalsy: true }), this.modelExplorerService.select('views')])
            .pipe(
    ...
    private async handleCalculateMinimumCoordinates(nodes: ModelViewNode[]) {
        function getMinForProperty(propertyName: keyof SubType<(typeof nodes)[0], number>) {
    ...
    private async handleSelectView(viewsById: Dictionary<ModelView>, viewId: string) {
