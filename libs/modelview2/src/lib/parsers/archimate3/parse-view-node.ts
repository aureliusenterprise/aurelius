...
export async function parseViewNode(node: any): Promise<ModelViewNode> {
    ...
    const styleOptions = {};
    styleOptions['textAlignment'] = parseTextAlignment(Number.parseInt(node['@textAlignment'], 10));
    styleOptions['textPosition'] = parseTextPosition(Number.parseInt(node['@textPosition'], 10));
