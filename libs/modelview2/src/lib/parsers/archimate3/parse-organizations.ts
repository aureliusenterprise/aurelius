...
async function parseOrganization(item: any, currentPath: string): Promise<Dictionary<string>> {
...
export async function parseOrganizations(organizations: any[] = [], currentPath = ''): Promise<Dictionary<string>> {
    const parsedOrganizations = organizations.map((item) => parseOrganization(item, currentPath));
