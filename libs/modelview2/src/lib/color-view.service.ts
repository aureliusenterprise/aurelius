...
export type DynamicColorPalette = 'diverging' | 'qualitative' | 'rainbow' | 'sequential';
...
export class ColorViewService extends BasicStore<ColorViewStoreContext> {
    ...
    private init() {
        ...
        combineLatest([this.select('bucketMapping'), this.select('colorPallette')])
            .pipe(
                switchMap(([bucketMapping, colorPallette]) => this.handleCreateLegend(bucketMapping, colorPallette)),
                untilDestroyed(this),
            )
            .subscribe();
        ...
        combineLatest([this.select('bucketMapping'), this.select('legend')])
            .pipe(
                switchMap(([bucketMapping, legend]) => this.handleCreateColorMapping(bucketMapping, legend)),
                untilDestroyed(this),
            )
            .subscribe();
    }
    ...
    private async handleCreateColorMapping(bucketMapping: Dictionary<string>, legend: Dictionary<string>) {
        const colorMapping = {};
    ...
    private async handleCreateLegend(bucketMapping: Dictionary<string>, colorPallette: ColorPallette) {
