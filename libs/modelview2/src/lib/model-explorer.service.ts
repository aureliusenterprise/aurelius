...
import { BidirectionalMultiMap, fromEntries, iterate, untilDestroyed } from '@models4insight/utils';
...
export class ModelExplorerService extends BasicStore<ModelExplorerStoreContext> {
    ...
    private init() {
        combineLatest([this.select('elements'), this.select('relations'), this.select('views')])
            .pipe(
                switchMap(([elements, relations, views]) => this.buildModelEntityIndex(elements, relations, views)),
                untilDestroyed(this),
            )
            .subscribe();
    ...
    private async buildConceptRelationMap(relations: Dictionary<ModelRelation>) {
        ...
        const conceptsByRelationId = fromEntries(iterate(mapping.mapping.associations())),
            relationsByConceptId = fromEntries(iterate(mapping.inverse.associations()));
    ...
    private async buildEntityViewMap(views: Dictionary<ModelView>) {
        ...
        const entitiesByViewId = fromEntries(iterate(mapping.mapping.associations())),
    ...
    private async buildModelEntityIndex(
        ...
        function buildViewEntityIndex() {
            return Object.values(views).reduce((acc, view) => ({ ...acc, ...view.nodes, ...view.connections }), {});
        }
