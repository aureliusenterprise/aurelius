...
export class I18nService extends BasicStore<I18nStoreContext> {
    ...
    constructor(
        ...
        super({
            defaultState: {
                ...
                userLanguage: localStorage.getItem(languageKey) || translateService.getBrowserCultureLang(),
            },
            name: 'I18nService',
            storeService,
        });
    ...
    init() {
        ...
        this.translateService.onLangChange.pipe(pluck('lang')).subscribe((lang: string) =>
            this.update({
        ...
        combineLatest([this.select('userLanguage'), this.select('supportedLanguages'), this.select('defaultLanguage')])
            .pipe(
                map(([userLang, supportedLangs, defaultLang]) => {
                    ...
                    if (userLang && !isSupportedLanguage) {
                        ...
                        lang = supportedLangs.find((supportedLanguage) => supportedLanguage.startsWith(noRegion)) || '';
