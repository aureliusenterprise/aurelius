 ...
 */
export const defaultResolver: MemoResolver<(arg0: string, ...args: any[]) => any> = (...args) => args[0];

/**
 * Function that builds a cache key from its arguments.
 */
export type MemoResolver<T extends Memoizable> = (...args: Parameters<T>) => string;
...
export function memoize<T extends Memoizable>(
    func: T,
    { cacheLimit = defaultCacheLimit, resolver = defaultResolver }: MemoizeOptions<T> = {},
): Memoized<T> {
