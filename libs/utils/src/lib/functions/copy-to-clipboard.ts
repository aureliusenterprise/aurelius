...
async function fallbackCopyValueToClipboard(value: string) {
    ...
    return new Promise<void>((resolve, reject) => {
        try {
            document.execCommand('copy') ? resolve() : reject(`Value ${value} could not be copied to the clipboard!`);
        } catch (e) {
            reject(e);
        } finally {
...
export async function copyToClipboard(value: string): Promise<void> {
    ...
    return navigator.clipboard ? navigator.clipboard.writeText(value) : fallbackCopyValueToClipboard(value);
}
