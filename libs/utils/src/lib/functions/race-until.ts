...
export async function raceUntil<T>(
    ...
    const conditionalPromises = promises.map(
        async (promise) =>
            new Promise<T>((resolve, reject) => promise.then((value) => predicate(value) && resolve(value), reject)),
    );

    return Promise.race<T>([...conditionalPromises, Promise.all(promises).then(() => defaultValue)]);
}
