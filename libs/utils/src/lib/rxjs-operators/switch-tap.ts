...
export function switchTap<T, R>(arg: Observable<T> | ((x: T) => Observable<R>)): MonoTypeOperatorFunction<T> {
    const next: (x: any) => Observable<T | R> = typeof arg === 'function' ? arg : (x: any): Observable<T> => arg;
    return switchMap((value) => concat(next(value).pipe(ignoreElements()), of(value)));
}
