...
export function MonitorAsync<T extends (...args: any[]) => Promise<any> | Observable<any>>(
    indicatorProperty: string | string[],
    { onComplete, store }: MonitorAsyncOptions = {},
) {
    return function (target: BasicStore<any>, key: string | symbol, descriptor: TypedPropertyDescriptor<T>) {
        const original = descriptor.value;
        descriptor.value = function (this: Function, ...args: any[]) {
            const originalResult = original.apply(this, args);
            return (store ?? target).monitorAsync.apply(this, [originalResult, indicatorProperty, onComplete]);
        } as any;
        return descriptor;
    };
}
 ...
 */
export abstract class BasicStore<CONTEXT extends StoreContext = {}> extends AbstractStore<CONTEXT, Action<CONTEXT>> {
    ...
    delete<P extends keyof CONTEXT>(options?: { description?: string; path?: [P] }): void;
    delete<P extends keyof CONTEXT, Q extends keyof CONTEXT[P]>(options?: {
        description?: string;
        path?: [P, Q];
    }): void;
    delete<P extends keyof CONTEXT, Q extends keyof CONTEXT[P], R extends keyof CONTEXT[P][Q]>(options?: {
        description?: string;
        path?: [P, Q, R];
    }): void;
    delete<
    ...
    delete(options: { description?: string; path?: (string | number)[] } = {}): void {
        this.dispatch({
    ...
    monitorAsync<L extends keyof CONTEXT, K extends keyof SubType<CONTEXT[L], boolean>, R>(
        operation: Observable<R>,
    ...
    monitorAsync<
        ...
        R,
    >(operation: Observable<R>, indicatorProperty: [L, M, K], onComplete?: Subject<void>): Observable<R>;
    monitorAsync<
        L extends keyof CONTEXT,
        M extends keyof CONTEXT[L],
        ...
        R,
    >(operation: Observable<R>, indicatorProperty: [L, M, N, K], onComplete?: Subject<void>): Observable<R>;
    monitorAsync<
        L extends keyof CONTEXT,
        M extends keyof CONTEXT[L],
        N extends keyof CONTEXT[L][M],
        ...
        R,
    >(operation: Observable<R>, indicatorProperty: [L, M, N, O, K], onComplete?: Subject<void>): Observable<R>;
    monitorAsync<R>(operation: Observable<R>, indicatorProperty: any, onComplete?: Subject<void>): Observable<R> {
        if (!Array.isArray(indicatorProperty)) {
            indicatorProperty = [indicatorProperty];
        }
        const setIndicatorState = (state: boolean) =>
            this.update({
                description: `Set ${indicatorProperty} to ${Boolean(state).toString().toUpperCase()}`,
     ...
     * @param {any} options.payload           The new state
     */
    update(options?: { description?: string; path?: []; payload?: Partial<CONTEXT> }): void;
    ...
    update<P extends keyof CONTEXT, Q extends keyof CONTEXT[P], R extends keyof CONTEXT[P][Q]>(options?: {
        description?: string;
        path?: [P, Q, R];
    ...
    update<
        ...
        R extends keyof CONTEXT[P][Q],
        S extends keyof CONTEXT[P][Q][R],
    >(options?: { description?: string; path?: [P, Q, R, S]; payload?: Partial<CONTEXT[P][Q][R][S]> }): void;
    update(
        options: {
            description?: string;
            path?: (string | number)[];
