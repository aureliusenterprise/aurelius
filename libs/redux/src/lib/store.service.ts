...
export class StoreService {
    private register$: Subject<Store<StoreContext>> = new Subject<Store<StoreContext>>();
    private snapshot$: BehaviorSubject<StoreSnapshot> = new BehaviorSubject<StoreSnapshot>({});
    ...
    private init() {
        ...
        this.register$
            .pipe(
                mergeMap((store) => store.state.pipe(map((state) => ({ [store.name]: state })))),
                scan((snapshot, stateMap) => ({ ...snapshot, ...stateMap }), this.snapshot$.value),
            )
