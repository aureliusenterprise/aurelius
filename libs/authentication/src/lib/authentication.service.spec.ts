...
describe('AuthenticationService', () => {
    ...
    describe('login', () => {
        it('should authenticate user', fakeAsync(() => {
            ...
            const isAuthenticated = authenticationService
                .isAuthenticated()
                .pipe(tap((isAuthenticated: boolean) => expect(isAuthenticated).toBeFalsy()));
            ...
            loginRequest.subscribe((isAuthenticated: boolean) => expect(isAuthenticated).toBeTruthy());
        }));

        it('should return credentials', fakeAsync(() => {
            const loginRequest = authenticationService.login();

            const credentialsRequest = loginRequest.pipe(switchMapTo(authenticationService.credentials()));
    ...
    describe('logout', () => {
        it('should clear user authentication', fakeAsync(() => {
            ...
            const credentialsRequest = logoutRequest.pipe(switchMapTo(authenticationService.credentials()));

            tick();

            credentialsRequest.subscribe((credentials: Credentials) => expect(credentials).toBeNull());
        }));
    });
});
