...
export class ShellService extends BasicStore<ShellStoreContext> implements OnDestroy {
    ...
    private init() {
        ...
        const routes = this.router.config.filter(
            (route) => route.redirectTo === undefined && !(route.data && route.data.hidden),
        );
        ...
        this.router.events.pipe(untilDestroyed(this)).subscribe((event) => this.navigationInterceptor(event));
        ...
        this.router.events
            .pipe(
                ...
                concatMap(([previousRoute, currentRoute]) =>
                    this.handleTrackPreviousRoute(previousRoute as NavigationEnd, currentRoute as NavigationEnd),
                ),
                untilDestroyed(this),
            )
            .subscribe();
        ...
        if (this.swUpdate.isEnabled) {
            ...
            const beforeInstallPrompt$ = fromEvent<BeforeInstallPromptEvent>(window, 'beforeinstallprompt').pipe(
                publish(),
            ) as ConnectableObservable<BeforeInstallPromptEvent>;
    ...
    private async handleTrackPreviousRoute(previousRoute: NavigationEnd, currentRoute: NavigationEnd) {
