...
def test__handle_relationship_audit_new_parent_breadcrumbs_applied() -> None:
    ...
    # New domain has its own breadcrumbs from a grandparent
    current_domain_document = AppSearchDocument(
        guid="domain-new",
        typename="m4i_data_domain",
        name="New Domain",
        referenceablequalifiedname="new_domain",
        breadcrumbguid=["grandparent"],
        breadcrumbname=["Grandparent Domain"],
        breadcrumbtype=["m4i_data_domain"],
    )

    # Entity currently has breadcrumbs from old parent
    current_entity_document = AppSearchDocument(
        guid="entity-1",
        typename="m4i_data_entity",
        name="Test Entity",
        referenceablequalifiedname="test_entity",
    ...
    with patch(
        ...
        with patch(
            ...
            with patch(
                ...
                # Breadcrumbs should now reflect the new parent's chain
                assert updated_entity.breadcrumbguid == ["grandparent", "domain-new"]
                assert updated_entity.breadcrumbname == ["Grandparent Domain", "New Domain"]
                assert updated_entity.breadcrumbtype == ["m4i_data_domain", "m4i_data_domain"]
