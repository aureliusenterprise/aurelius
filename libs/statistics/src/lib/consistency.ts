...
export async function consistency<A, B>(
    ...
    const sortedPairs = orderBy(Array.from(pairs(referenceValues, otherValues)), [0, 1]);
   ...
   */
    const isConsistentPair$ = range(1, Math.max(referenceValues.length, otherValues.length)).map(async (index) => {
    ...
    if (shortCirquiting) {
        ...
        const hasAllConsistentPairs = await raceUntil(isConsistentPair$, (isConsistent) => !isConsistent, true);
        ...
        return sortedPairs.length
            ? (isConsistentPair.filter((isConsistent) => isConsistent).length + 1) / sortedPairs.length
            : 1;
    }
}
