...
from m4i_data_management.core.cdc.change_types import CDCChangeType
from m4i_data_management.core.cdc.columns import CHANGE_TYPE_COLUMN
from m4i_data_management.core.cdc.compare_datasets.compare_datasets import compare_datasets
...
def test__compare_datasets_annotates_changes(old_data, new_data):
    ...
    assert len(added.index) == 1  # type: ignore[union-attr]
    assert len(removed.index) == 1  # type: ignore[union-attr]
    assert len(changed.index) == 1  # type: ignore[union-attr]
