...
def make_confluent_producer() -> Producer:
    """
    Returns a connection to Confluent Kafka in the Data Management Platform that
    can be used to push change events.

    It is assumed that sasl is used, if this is not the case please set the in
    the config key "sasl_flag" to False
    """
    ...
    if sasl_flag:
        username, password = config.get_many(
            "confluent.auth.sasl.username", "confluent.auth.sasl.password", all_required=True
