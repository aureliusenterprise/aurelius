...
def conditional_value(
    ...
    """
    Checks whether the values in the given `value_column` match (one of) the expected
    value(s) for a given key in the `key_column`.
    ...
    def check(row):
        ...
        if isna(key):  # type: ignore[arg-type]
            return 0
    ...
    rows_to_check = data[data[key_column].isin(value_mapping)]  # type: ignore[union-attr]

    if len(rows_to_check) == 0:  # type: ignore[arg-type]
        return Series()

    return rows_to_check[[value_column, key_column]].apply(check, axis=1)  # type: ignore[return-value]
