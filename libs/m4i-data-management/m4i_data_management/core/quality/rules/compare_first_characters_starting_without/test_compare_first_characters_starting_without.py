...
def test__compare_first_characters_starting_without_match_without():
    ...
    data = DataFrame([{"id": "NL.xxx", "name": "NL.xxx"}])
...
def test__compare_first_characters_starting_without_notmatch_without():
    ...
    data = DataFrame([{"id": "NL.xxx", "name": "BE.xxx"}])
...
def test__compare_first_characters_starting_without_match_with():
    ...
    data = DataFrame([{"id": "BE.xxx", "name": "BE.xxx"}])
...
def test__result_index_matches_original_index():
    ...
    result = compare_first_characters_starting_without(data, "id", "name", 2, "BE")  # type: ignore[arg-type]

    assert all(data.index == result.index)  # type: ignore[arg-type]
