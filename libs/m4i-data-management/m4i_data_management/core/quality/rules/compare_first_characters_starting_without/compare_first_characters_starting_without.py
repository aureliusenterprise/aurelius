...
def compare_first_characters_starting_without(
    data: DataFrame,
    ...
    *prefixes: str,
    ...
    and `second_column_name` are similar, and if  `column_name` does not start with
    any of the given `prefixes`, and if the values are None or NaN.
    ...
    same_first_char = compare_first_characters(
        data, first_column_name, second_column_name, number_of_characters
    )
    start_with = starts_with(data, first_column_name, *prefixes)

    return DataFrame({"same_first_char": same_first_char, "start_with": start_with}).apply(check, axis=1)  # type: ignore[return-value]
