...
def atlas_get_quality_rules_dataframe(data: List[BusinessDataQuality] = []):
    """Returns empty DataFrame with column names for data quality rules.

    Column names are based on the data dictionary.
    """
    columns_names = [
        "id",
...
async def get_dataset(
    dataset_name: str, access_token: str, dataset_atlas_guid: Optional[str] = None
) -> BusinessDataset:
    if dataset_atlas_guid is None:
        ...
        if len(dataset_entity_headers) > 1:
            print(
                f"More than one dataset with the name '{dataset_name}' was found. "
                "Please provide the atlas guid of the dataset as well."
            )
            exit(1)
        if len(dataset_entity_headers) == 0:
            print(
                f"No dataset entity with the name '{dataset_name}' was found. "
                "Please provide the atlas guid of the dataset as well."
            )
            exit(1)
    ...
    return await get_entity_by_guid(
        guid=dataset_atlas_guid, entity_type=BusinessDataset, access_token=access_token
    )
...
async def get_quality_rules(fields: List[BusinessField], access_token: str) -> List[BusinessDataQuality]:
    quality_rules = [
        get_entity_by_guid(
            guid=quality_rule["guid"],  # type: ignore[arg-type]
            entity_type=BusinessDataQuality,
            access_token=access_token,
        )
        for field in fields
        for quality_rule in field.relationship_attributes["dataQuality"]  # type: ignore[index]
    ]
...
async def get_quality_rules_from_atlas(
    dataset_name: str, dataset_atlas_guid: Optional[str] = None
) -> DataFrame:
    """Gets all data quality rules for a dataset and returns them as a dataframe.

    Given a dataset_name or dataset_atlas_guid, gets all associated data quality rules.
    ...
    return atlas_get_quality_rules_dataframe(quality_rules)  # type: ignore[return-value]
