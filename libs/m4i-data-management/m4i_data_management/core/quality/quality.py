...
class Quality:
    ...
    def run(self):
        ...
        3. Apply the data quality rules to the dataset, which results in an overall
            data quality summary as well as a list of compliant and non-compliant rows
        ...
        summary, compliant, non_compliant = evaluate_data_quality_rules(data=data, rules=rules)

        log.info(
            f"Evaluated {len(summary.index)} data quality rules; "
            f"found {len(compliant.index)} compliant rows and "
            f"{len(non_compliant.index)} non-compliant rows"
        )

        metadata = self.get_metadata()

        log.info(f"Retrieved {len(metadata.index)} rows of metadata from the data dictionary")
        ...
        log.info(
            f"Annotated {len(summary.index) + len(compliant.index) + len(non_compliant.index)} "
            f"results with metadata from the data dictionary"
        )

        self.propagate(summary, compliant, non_compliant)

        log.info(
            f"Propagated {len(summary.index)} data quality test summaries and "
            f"{len(compliant.index) + len(non_compliant)} test details"
        )
