...
from ..utils import find_changed_rows, find_different_rows

log = logging.getLogger(__name__)


def compare_datasets(
    old: DataFrame, new: DataFrame, is_equal: Callable[[Series, Series], bool] = are_hashes_equal
) -> DataFrame:
    """
    ...
    log.info(f"Comparing {len(new.index)} new records against {len(old.index)} old records")
