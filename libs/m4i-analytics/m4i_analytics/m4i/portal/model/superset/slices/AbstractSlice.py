...
class AbstractSlice:
    ...
    def __init__(
        ...
        self._init_params(**(json.loads(params)))  # type: ignore[reportGeneralTypeIssues]
    ...
    def directColumnDependencies(self):
        table_columns_table = Table("table_columns", self.db_metadata, autoload_with=self.db_con)  # type: ignore[arg-type]
        table_columns = [
            row
    ...
    def columnNames(self):
        return list(
            set(
                [column for metric in self.getMetrics() for column in self.columnsFromSQL(metric[7])]
    ...
    def getMetrics(self):
        table = Table("sql_metrics", self.db_metadata, autoload_with=self.db_con)  # type: ignore[arg-type]
