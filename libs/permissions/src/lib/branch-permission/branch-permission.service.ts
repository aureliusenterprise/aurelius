...
export class BranchPermissionService extends BasicStore<BranchPermissionStoreContext> {
     ...
     */
    checkPermission(branchName: string, requiredPermissionLevel: BranchPermissionLevel): Observable<boolean> {
        this.checkPermission$.next(branchName);

        const branchPermissionLevel = this.projectService
            .select('projectId')
            .pipe(switchMap((projectId) => this.select(['permissions', projectId, branchName])));
